...
function backupSettingsSliceSrc() {
  return (extractFunctionSource(html, 'collectBackupSettingsSnapshot') || '') + '\n' + (extractFunctionSource(html, '_buildFullBackupData') || '');
}

function requiredBusinessAdapterSrc() {
...
function p1cValidatorSrc(srcHtml) {
  ...
  return [
    ...
    fromSchema ? fromSchema[0] : "var REQUIRED_BACKUP_COLLECTIONS_FROM_SCHEMA = { 1: ['sales', 'parts', 'accounts'] };",
...
test('اسکین باید در بک‌آپ appearance ذخیره و هنگام بازگردانی اعمال شود', () => {
  const settingsSrc = backupSettingsSliceSrc();
  assertTrue(extractFunctionSource(html, '_buildFullBackupData') !== null, 'تابع _buildFullBackupData پیدا نشد');
  assertContainsString(settingsSrc, "skin: localStorage.getItem('laegh_skin')", 'appearance.skin باید در بک‌آپ ذخیره شود');
...
test('عمق سه‌بعدی باید در بک‌آپ و applyAppearanceSettings باشد و پیش‌فرض روشن باشد', () => {
  const settingsSrc = backupSettingsSliceSrc();
  assertContainsString(settingsSrc, "depth3d: localStorage.getItem('laegh_depth3d')", 'depth3d باید در بک‌آپ ذخیره شود');
...
test('بک‌آپ باید حالت منو/شکل/پس‌زمینه‌ها/شورتکات را ذخیره و بازگردانی کند', () => {
  const settingsSrc = backupSettingsSliceSrc();
  assertContainsString(settingsSrc, "sbMode: localStorage.getItem('laegh_sb_mode')", 'sbMode در بک‌آپ نیست');
  assertContainsString(settingsSrc, "navShape: localStorage.getItem('laegh_nav_shape')", 'navShape در بک‌آپ نیست');
  assertContainsString(settingsSrc, "sbBg: localStorage.getItem('laegh_sb_bg')", 'sbBg در بک‌آپ نیست');
  assertContainsString(settingsSrc, "mainBg: localStorage.getItem('laegh_main_bg')", 'mainBg در بک‌آپ نیست');
  assertContainsString(settingsSrc, "dashBg: localStorage.getItem('laegh_dash_bg')", 'dashBg در بک‌آپ نیست');
  assertContainsString(settingsSrc, "dashShortcuts: localStorage.getItem('laegh_dash_shortcuts')", 'dashShortcuts در بک‌آپ نیست');
...
test('رنگ فونت باید در بک‌آپ، بازگردانی، حفاظت و ظاهر ذخیره شود', () => {
  const settingsSrc = backupSettingsSliceSrc();
  assertContainsString(settingsSrc, "textColor: localStorage.getItem('laegh_text_color')", 'textColor باید در بک‌آپ باشد');
...
test('داشبورد باید رنگ متمایز پیش‌فرض و کنترل setDashTint داشته باشد', () => {
  ...
  const settingsSrc = backupSettingsSliceSrc();
  assertContainsString(settingsSrc, "dashTint: localStorage.getItem('laegh_dash_tint')", 'dashTint باید در بک‌آپ باشد');
});
...
test('برچسب پستی: مسیر HTML قدیمی می‌ماند و مسیر بومی صریح و جدا است', () => {
  ...
  assertContainsString(nativeSvc, 'PostalRtlWrapFormat', 'برچسب پستی باید StringFormat جدا از فاکتور داشته باشد');
  assertContainsString(nativeSvc, 'PostalLtrWrapFormat', 'اعداد پستی باید LTR جدا از LtrFormat فاکتور داشته باشند');
  assertContainsString(nativeSvc, 'PostalLabelFieldPlan.Sender', 'چینش فرستنده باید از طرح Core بیاید');
  assertContainsString(nativeSvc, 'StringTrimming.None', 'رندر پستی نباید ellipsis بگذارد');
  assertTrue(nativeSvc.indexOf('telLine') < 0, 'نام مسئول نباید به مقدار LTR تلفن چسبانده شود');
  assertContainsString(nativeSvc, 'Trimming = StringTrimming.EllipsisCharacter', 'RtlFormat فاکتور باید ellipsis را نگه دارد');
  const rtlFactory = nativeSvc.slice(nativeSvc.indexOf('private static StringFormat RtlFormat()'));
  assertTrue(rtlFactory.indexOf('EllipsisCharacter') >= 0, 'کارخانه RtlFormat فاکتور نباید عوض شود');
  const postalWrap = nativeSvc.slice(nativeSvc.indexOf('private static StringFormat PostalRtlWrapFormat()'), nativeSvc.indexOf('private static StringFormat RtlFormat()'));
  assertTrue(postalWrap.indexOf('EllipsisCharacter') < 0, 'فرمت پستی نباید EllipsisCharacter داشته باشد');
  const planSrc = fs.readFileSync(path.join(path.dirname(filePath), 'desktop', 'Sirman.Core', 'Printing', 'PostalLabelFieldPlan.cs'), 'utf8');
  assertContainsString(planSrc, 'PostalLabelDrawnField.Person', 'مسئول فرستنده باید فیلد RTL جدا باشد');
  assertTrue(planSrc.indexOf('s.Tel +') < 0, 'طرح پستی نباید تلفن و مسئول را یکی کند');
...
test('رنگ سرتیتر منو باید در ظاهر، CSS، بک‌آپ و حفاظت باشد', () => {
  ...
  const settingsSrc = backupSettingsSliceSrc();
  assertContainsString(settingsSrc, "headingColor: localStorage.getItem('laegh_heading_color')", 'headingColor باید در بک‌آپ باشد');
...
console.log('');
console.log('📋 گروه: P1C-3 اعتبارسنج fail-closed فروش (sales, Schema ≥۱)');

function p1c3SyntheticSale(){
  return { id:'SL-TEST', saleUid:'SALEUID-000001', items:[], status:'final', accRef:'', accountSel:'' };
}

function p1c3RunRestoreGate(mode, backup, liveSales){
  const valSrc = p1cValidatorSrc(html);
  const wantsSrc = extractFunctionSource(html, '_restoreWants');
  const replaceSrc = extractFunctionSource(html, 'applyBackupReplaceSections');
  const mergeSrc = extractFunctionSource(html, 'applyBackupMergeSections');
  const selectiveSrc = extractFunctionSource(html, 'applyBackupSelective');
  const fns = valSrc + '\n' + wantsSrc + '\n' + replaceSrc + '\n' + mergeSrc + '\n' + selectiveSrc;
  const runner = new Function('backup','mode','liveSales', fns + `
    var mutations = { sv:0, svWarr:0, svSales:0, snapshot:0, auditOk:0, auditErr:0, ls:0 };
    var sales = (liveSales||[]).map(function(x){ return JSON.parse(JSON.stringify(x)); });
    var invoices = []; var warranties = []; var products = []; var inventory = {}; var phonebook = []; var pb = [];
    var parts = []; var services = []; var svcs = []; var tasks = [];
    var accounts = []; var defectiveStock = []; var warehouseDocs = []; var stockMoves = [];
    var warehouses = []; var daqi = []; var daqiWarehouse = []; var daqiVouchers = []; var postalHistory = [];
    var userRoles = []; var saleCtr = 1; var saleUidCtr = 0; var invCtr = 1; var invoiceUidCtr = 0;
    var logoSrc = ''; var senderInfo = {}; var acH = {};
    var localStorage = { setItem:function(){ mutations.ls++; }, getItem:function(){ return null; } };
    function sv(){ mutations.sv++; }
    function svParts(){} function svSvcs(){}
    function svSales(){ mutations.svSales++; }
    function svWarr(){ mutations.svWarr++; }
    function svTasks(){} function svDefective(){} function svAccounts(){} function svWarehouses(){}
    function _buildFullBackupData(){ mutations.snapshot++; return { warranties: warranties, invoices: invoices, sales: sales }; }
    function saveSafetySnapshot(){ mutations.snapshot++; }
    function logBackupAudit(op, result){ if(result==='ok') mutations.auditOk++; else mutations.auditErr++; }
    function addDbgEntry(){} function ntf(){} function auditUser(){} function alert(){}
    function emit(){} function getNum(){} function renderSaved(){} function renderProds(){} function renderInv(){}
    ...
    function renderWarList(){} function renderDataStats(){} function renderTasks(){} function renderSidebarBadges(){}
    var threw = null;
    var before = JSON.stringify(sales);
    var beforeWar = JSON.stringify(warranties);
    var beforeInv = JSON.stringify(invoices);
    try {
      if(mode==='replace') applyBackupReplaceSections(backup, ['sales']);
      else if(mode==='merge') applyBackupMergeSections(backup, ['sales']);
      else applyBackupSelective(backup, ['sales'], (mode==='selective-replace' ? 'replace' : 'merge'));
    } catch(e){ threw = String(e && e.message || e); }
    return {
      threw: threw,
      live: JSON.parse(JSON.stringify(sales)),
      liveUnchanged: JSON.stringify(sales) === before,
      warUnchanged: JSON.stringify(warranties) === beforeWar,
      invUnchanged: JSON.stringify(invoices) === beforeInv,
      mutations: mutations
    };
  `);
  return runner(backup, mode, liveSales || [{id:'LIVE-SALE', saleUid:'SALEUID-000099'}]);
}

test('P1C-3 T1: Schema 1 + sales غایب باید FAIL شود', () => {
  const v = loadP1CValidator().validateRequiredBackupCollections({schemaVersion:1, warranties:[], invoices:[]});
  assertEqual(v.ok, false, 'Schema ≥۱ بدون sales باید نامعتبر باشد');
  assertTrue(v.missingRequiredCollections.indexOf('sales')>=0, 'دلیل باید missing sales باشد');
...
test('P1C-3 T2: Schema 1 + sales=null باید FAIL شود', () => {
  const v = loadP1CValidator().validateRequiredBackupCollections({schemaVersion:1, warranties:[], invoices:[], sales:null});
  assertEqual(v.ok, false, 'null باید نامعتبر باشد');
  assertTrue(v.invalidCollections.indexOf('sales')>=0, 'null باید invalidCollections باشد نه missing تفسیرشده به []');
});

test('P1C-3 T3: Schema 1 + sales غیرآرایه باید FAIL شود', () => {
  const v = loadP1CValidator().validateRequiredBackupCollections({schemaVersion:1, warranties:[], invoices:[], sales:'invalid'});
  assertEqual(v.ok, false, 'رشته باید نامعتبر باشد');
  assertTrue(v.invalidCollections.indexOf('sales')>=0, 'نوع نامعتبر باید در invalidCollections باشد');
});

test('P1C-3 T4: Schema 1 + sales=[] باید PASS شود', () => {
  const input = {schemaVersion:1, warranties:[], invoices:[], sales:[], parts:[], accounts:[]};
  const v = loadP1CValidator().validateRequiredBackupCollections(input);
  assertEqual(v.ok, true, 'آرایه خالی صریح باید معتبر باشد');
  assertEqual(v.missingRequiredCollections.length, 0, '[] نباید missing باشد');
  assertTrue(Array.isArray(input.sales) && input.sales.length===0, '[] باید [] بماند');
});

test('P1C-3 T5: Schema 1 + sales با رکورد مصنوعی معتبر باید PASS شود', () => {
  const v = loadP1CValidator().validateRequiredBackupCollections({
    schemaVersion:1, warranties:[], invoices:[], sales:[p1c3SyntheticSale()], parts:[], accounts:[]
  });
  assertEqual(v.ok, true, 'آرایه دارای رکورد باید معتبر باشد');
  assertEqual(v.invalidCollections.length, 0, 'رکورد مصنوعی نباید invalid باشد');
});

test('P1C-3 T6: Schema 0 + sales غایب باید سازگار PASS شود', () => {
  const input = { version:'2.0', warranties:[], invoices:[] };
  const v = loadP1CValidator().validateRequiredBackupCollections(input);
  assertEqual(v.ok, true, 'Schema 0 بدون sales باید معتبر بماند');
  assertTrue(v.missingRequiredCollections.indexOf('sales')<0, 'sales نباید برای Schema 0 الزامی شود');
  assertEqual(Object.prototype.hasOwnProperty.call(input,'sales'), false, 'validator نباید کلید sales بسازد');
});

test('P1C-3 T7: Schema 0 + sales معتبر باید PASS شود', () => {
  const v = loadP1CValidator().validateRequiredBackupCollections({
    version:'7.0', warranties:[], invoices:[], sales:[p1c3SyntheticSale()]
  });
  assertEqual(v.ok, true, 'Schema 0 با sales صریح باید معتبر باشد');
  assertEqual(v.invalidCollections.length, 0, 'رکورد فروش Schema 0 نباید invalid باشد');
});

test('P1C-3 T8: Schema 1 + sales غایب باید MERGE را مسدود کند', () => {
  const backup = { schemaVersion:1, warranties:[], invoices:[], products:[{code:'A'}], phonebook:[], parts:[] };
  const r = p1c3RunRestoreGate('merge', backup);
  assertTrue(!!r.threw && /sales/.test(r.threw), 'merge باید به‌خاطر sales غایب throw کند');
  assertEqual(r.liveUnchanged, true, 'فروش زنده نباید عوض شود');
  assertEqual(r.mutations.sv, 0, 'sv نباید صدا شود');
  assertEqual(r.mutations.svSales, 0, 'svSales نباید صدا شود');
});

test('P1C-3 T9: Schema 1 + sales غایب باید REPLACE را مسدود کند', () => {
  const backup = { schemaVersion:1, warranties:[], invoices:[], products:[], phonebook:[], parts:[] };
  const r = p1c3RunRestoreGate('replace', backup);
  assertTrue(!!r.threw && /sales/.test(r.threw), 'replace باید به‌خاطر sales غایب throw کند');
  assertEqual(r.liveUnchanged, true, 'فروش زنده نباید عوض شود');
  assertEqual(r.mutations.snapshot, 0, 'replace مستقیم نباید به snapshot برسد');
});

test('P1C-3 T10: شکست اعتبارسنجی Schema 1 نباید هیچ mutation زنده ایجاد کند', () => {
  const rSel = p1c3RunRestoreGate('selective-replace', {schemaVersion:1, warranties:[{id:'W1'}], invoices:[{num:'1'}]});
  assertTrue(!!rSel.threw, 'selective باید متوقف شود');
  assertEqual(rSel.liveUnchanged, true, 'آرایه زنده sales نباید عوض شود');
  assertEqual(rSel.warUnchanged, true, 'warranties زنده نباید عوض شود');
...
test('P1C-3 T11: فیکسچر قدیمی Schema 0 بدون sales باید همان رفتار سازگار قبلی را حفظ کند', () => {
  const backup = { version:'10.3.29', invoices:[{num:'1', seller:'A'}], phonebook:[{fn:'علی', ln:'رضایی'}], warranties:[{id:'W1'}] };
  const v = loadP1CValidator().validateRequiredBackupCollections(backup);
  assertEqual(v.ok, true, 'فیکسچر ۱۰.۳.۲۹ بدون sales باید از اعتبارسنج عبور کند');
  assertTrue(v.missingRequiredCollections.indexOf('sales')<0, 'sales نباید missing Schema 0 باشد');
  const migrateBackup = loadMigrateBackupFn();
  const migrated = migrateBackup(JSON.parse(JSON.stringify(backup)));
  assertTrue(Array.isArray(migrated.data.sales) && migrated.data.sales.length===0, 'migrateBackup Schema 0 باید sales=[] سازگاری بسازد');
  assertTrue((migrated.log||[]).some(function(x){ return /نسخه قدیمی/.test(x); }), 'لاگ باید سازگاری نسخه قدیمی را ثبت کند');
  const eng = loadBackupEngine(html);
  const originalJson = JSON.stringify(backup);
  const mig = eng.BackupEngine.migrateSchema(backup, 1);
  assertTrue(mig.ok, 'Migration ۰→۱ باید موفق باشد');
  assertTrue(Array.isArray(mig.data.sales) && mig.data.sales.length===0, '۰→۱ باید sales غایب Schema 0 را به [] تبدیل کند');
  assertTrue((mig.log||[]).some(function(x){ return /فروش غایب/.test(x); }), 'لاگ ۰→۱ باید تبدیل سازگاری فروش را ثبت کند');
  assertEqual(JSON.parse(originalJson).sales, undefined, 'فیکسچر اصلی نباید mutate شود');
  const r = p1c3RunRestoreGate('merge', backup, [{id:'LIVE-SALE'}]);
  assertEqual(r.threw, null, 'merge Schema 0 بدون sales نباید throw کند');
  assertEqual(r.liveUnchanged, true, 'ادغام بدون sales نباید فروش زنده را پاک کند');
});

test('P1C-3 T12: گارانتی غایب همچنان FAIL است', () => {
  const v = loadP1CValidator().validateRequiredBackupCollections({schemaVersion:1, invoices:[], sales:[]});
  ...
  const r = p1cRunRestoreGate('replace', {schemaVersion:1, invoices:[], sales:[]});
  assertTrue(!!r.threw && /warranties/.test(r.threw), 'replace بدون warranties باید مسدود بماند');
});

test('P1C-3 T13: فاکتور غایب همچنان FAIL است', () => {
  const v = loadP1CValidator().validateRequiredBackupCollections({schemaVersion:1, warranties:[], sales:[]});
  ...
  const r = p1c2RunRestoreGate('merge', {schemaVersion:1, warranties:[], sales:[]});
  assertTrue(!!r.threw && /invoices/.test(r.threw), 'merge بدون invoices باید مسدود بماند');
});
...
test('P1C-3 T14: warranties=[] همچنان PASS است', () => {
  ...
  const v1 = loadP1CValidator().validateRequiredBackupCollections({schemaVersion:1, warranties:[], invoices:[], sales:[], parts:[], accounts:[]});
...
test('P1C-3: migrateBackup نباید sales غایب Schema ≥۱ را به [] تبدیل کند', () => {
  const migrateBackup = loadMigrateBackupFn();
  const src = extractFunctionSource(html, 'migrateBackup');
  assertTrue(src.indexOf('fail-closed, Schema ≥۱') >= 0, 'مسیر fail-closed فروش Schema ≥۱ باید موجود باشد');
  const d = { schemaVersion:1, version:'1405.6.3α', warranties:[], invoices:[], products:[], inventory:{}, phonebook:[] };
  assertEqual(Object.prototype.hasOwnProperty.call(d,'sales'), false, 'ورودی نباید sales داشته باشد');
  const result = migrateBackup(d);
...
test('P1C-4 T5: Schema 1 + parts با رکورد مصنوعی معتبر باید PASS شود', () => {
  const v = loadP1CValidator().validateRequiredBackupCollections({
    schemaVersion:1, warranties:[], invoices:[], sales:[], parts:[p1c4SyntheticPart()], accounts:[]
  });
  assertEqual(v.ok, true, 'آرایه دارای رکورد باید معتبر باشد');
  assertEqual(v.invalidCollections.length, 0, 'رکورد مصنوعی نباید invalid باشد');
});
...
console.log('');
console.log('📋 گروه: P1C-5 اعتبارسنج fail-closed حساب‌ها (accounts, Schema ≥۱)');

function p1c5SyntheticAccount(){
  return { id:'ACC-TEST', number:'1', name:'حساب آزمایشی', desc:'', balance:0, transactions:[], createdAt:'' };
}

function p1c5RunRestoreGate(mode, backup, liveAccounts){
  const valSrc = p1cValidatorSrc(html);
  const wantsSrc = extractFunctionSource(html, '_restoreWants');
  const replaceSrc = extractFunctionSource(html, 'applyBackupReplaceSections');
  const mergeSrc = extractFunctionSource(html, 'applyBackupMergeSections');
  const selectiveSrc = extractFunctionSource(html, 'applyBackupSelective');
  const fns = valSrc + '\n' + wantsSrc + '\n' + replaceSrc + '\n' + mergeSrc + '\n' + selectiveSrc;
  const runner = new Function('backup','mode','liveAccounts', fns + `
    var mutations = { sv:0, svWarr:0, svSales:0, svParts:0, svAccounts:0, snapshot:0, auditOk:0, auditErr:0, ls:0 };
    var accounts = (liveAccounts||[]).map(function(x){ return JSON.parse(JSON.stringify(x)); });
    var invoices = []; var warranties = []; var products = []; var inventory = {}; var phonebook = []; var pb = [];
    var parts = []; var services = []; var svcs = []; var sales = []; var tasks = [];
    ...
    var threw = null;
    var before = JSON.stringify(accounts);
    var beforeWar = JSON.stringify(warranties);
    var beforeInv = JSON.stringify(invoices);
    var beforeSales = JSON.stringify(sales);
    var beforeParts = JSON.stringify(parts);
    try {
      if(mode==='replace') applyBackupReplaceSections(backup, ['accounts']);
      else if(mode==='merge') applyBackupMergeSections(backup, ['accounts']);
      else applyBackupSelective(backup, ['accounts'], (mode==='selective-replace' ? 'replace' : 'merge'));
    } catch(e){ threw = String(e && e.message || e); }
    return {
      threw: threw,
      live: JSON.parse(JSON.stringify(accounts)),
      liveUnchanged: JSON.stringify(accounts) === before,
      warUnchanged: JSON.stringify(warranties) === beforeWar,
      invUnchanged: JSON.stringify(invoices) === beforeInv,
      salesUnchanged: JSON.stringify(sales) === beforeSales,
      partsUnchanged: JSON.stringify(parts) === beforeParts,
      mutations: mutations
    };
  `);
  return runner(backup, mode, liveAccounts || [{id:'LIVE-ACC', name:'صندوق زنده', balance:10}]);
}

test('P1C-5 T1: Schema 1 + accounts غایب باید FAIL شود', () => {
  const v = loadP1CValidator().validateRequiredBackupCollections({schemaVersion:1, warranties:[], invoices:[], sales:[], parts:[]});
  assertEqual(v.ok, false, 'Schema ≥۱ بدون accounts باید نامعتبر باشد');
  assertTrue(v.missingRequiredCollections.indexOf('accounts')>=0, 'دلیل باید missing accounts باشد');
  assertTrue(v.missingRequiredCollections.indexOf('warranties')<0, 'warranties=[] نباید missing شود');
  assertTrue(v.missingRequiredCollections.indexOf('invoices')<0, 'invoices=[] نباید missing شود');
  assertTrue(v.missingRequiredCollections.indexOf('sales')<0, 'sales=[] نباید missing شود');
  assertTrue(v.missingRequiredCollections.indexOf('parts')<0, 'parts=[] نباید missing شود');
});

test('P1C-5 T2: Schema 1 + accounts=null باید FAIL شود', () => {
  const v = loadP1CValidator().validateRequiredBackupCollections({schemaVersion:1, warranties:[], invoices:[], sales:[], parts:[], accounts:null});
  assertEqual(v.ok, false, 'null باید نامعتبر باشد');
  assertTrue(v.invalidCollections.indexOf('accounts')>=0, 'null باید invalidCollections باشد نه missing تفسیرشده به []');
});

test('P1C-5 T3: Schema 1 + accounts غیرآرایه باید FAIL شود', () => {
  const v = loadP1CValidator().validateRequiredBackupCollections({schemaVersion:1, warranties:[], invoices:[], sales:[], parts:[], accounts:'invalid'});
  assertEqual(v.ok, false, 'رشته باید نامعتبر باشد');
  assertTrue(v.invalidCollections.indexOf('accounts')>=0, 'نوع نامعتبر باید در invalidCollections باشد');
});

test('P1C-5 T4: Schema 1 + accounts=[] باید PASS شود', () => {
  const input = {schemaVersion:1, warranties:[], invoices:[], sales:[], parts:[], accounts:[]};
  const v = loadP1CValidator().validateRequiredBackupCollections(input);
  assertEqual(v.ok, true, 'آرایه خالی صریح باید معتبر باشد');
  assertEqual(v.missingRequiredCollections.length, 0, '[] نباید missing باشد');
  assertTrue(Array.isArray(input.accounts) && input.accounts.length===0, '[] باید [] بماند');
});

test('P1C-5 T5: Schema 1 + accounts با رکورد مصنوعی معتبر باید PASS شود', () => {
  const v = loadP1CValidator().validateRequiredBackupCollections({
    schemaVersion:1, warranties:[], invoices:[], sales:[], parts:[], accounts:[p1c5SyntheticAccount()]
  });
  assertEqual(v.ok, true, 'آرایه دارای رکورد باید معتبر باشد');
  assertEqual(v.invalidCollections.length, 0, 'رکورد مصنوعی نباید invalid باشد');
});

test('P1C-5 T6: Schema 0 + accounts غایب باید سازگار PASS شود', () => {
  const input = { version:'2.0', warranties:[], invoices:[] };
  const v = loadP1CValidator().validateRequiredBackupCollections(input);
  assertEqual(v.ok, true, 'Schema 0 بدون accounts باید معتبر بماند');
  assertTrue(v.missingRequiredCollections.indexOf('accounts')<0, 'accounts نباید برای Schema 0 الزامی شود');
  assertEqual(Object.prototype.hasOwnProperty.call(input,'accounts'), false, 'validator نباید کلید accounts بسازد');
});

test('P1C-5 T7: Schema 1 + accounts غایب باید MERGE را مسدود کند', () => {
  const backup = { schemaVersion:1, warranties:[], invoices:[], sales:[], parts:[], products:[{code:'A'}], phonebook:[] };
  const r = p1c5RunRestoreGate('merge', backup);
  assertTrue(!!r.threw && /accounts/.test(r.threw), 'merge باید به‌خاطر accounts غایب throw کند');
  assertEqual(r.liveUnchanged, true, 'حساب زنده نباید عوض شود');
  assertEqual(r.mutations.sv, 0, 'sv نباید صدا شود');
  assertEqual(r.mutations.svAccounts, 0, 'svAccounts نباید صدا شود');
});

test('P1C-5 T8: Schema 1 + accounts غایب باید REPLACE را مسدود کند', () => {
  const backup = { schemaVersion:1, warranties:[], invoices:[], sales:[], parts:[], products:[], phonebook:[] };
  const r = p1c5RunRestoreGate('replace', backup);
  assertTrue(!!r.threw && /accounts/.test(r.threw), 'replace باید به‌خاطر accounts غایب throw کند');
  assertEqual(r.liveUnchanged, true, 'حساب زنده نباید عوض شود');
  assertEqual(r.mutations.snapshot, 0, 'replace مستقیم نباید به snapshot برسد');
});

test('P1C-5 T9: شکست اعتبارسنجی Schema 1 نباید هیچ mutation زنده ایجاد کند', () => {
  const rSel = p1c5RunRestoreGate('selective-replace', {schemaVersion:1, warranties:[{id:'W1'}], invoices:[{num:'1'}], sales:[], parts:[]});
  assertTrue(!!rSel.threw, 'selective باید متوقف شود');
  assertEqual(rSel.liveUnchanged, true, 'آرایه زنده accounts نباید عوض شود');
  assertEqual(rSel.warUnchanged, true, 'warranties زنده نباید عوض شود');
  assertEqual(rSel.invUnchanged, true, 'invoices زنده نباید عوض شود');
  assertEqual(rSel.salesUnchanged, true, 'sales زنده نباید عوض شود');
  assertEqual(rSel.partsUnchanged, true, 'parts زنده نباید عوض شود');
  assertEqual(rSel.mutations.sv, 0, 'sv نباید صدا شود');
  assertEqual(rSel.mutations.svWarr, 0, 'svWarr نباید صدا شود');
  assertEqual(rSel.mutations.svSales, 0, 'svSales نباید صدا شود');
  assertEqual(rSel.mutations.svParts, 0, 'svParts نباید صدا شود');
  assertEqual(rSel.mutations.svAccounts, 0, 'svAccounts نباید صدا شود');
  assertEqual(rSel.mutations.snapshot, 0, 'snapshot ایمنی نباید نوشته شود');
  assertEqual(rSel.mutations.auditOk, 0, 'audit موفقیت نباید ثبت شود');
  assertEqual(rSel.mutations.ls, 0, 'localStorage نباید نوشته شود');
});

test('P1C-5 T10: migrateBackup نباید accounts غایب Schema ≥۱ را به [] تبدیل کند', () => {
  const migrateBackup = loadMigrateBackupFn();
  const src = extractFunctionSource(html, 'migrateBackup');
  assertTrue(src.indexOf('accountsSchemaVer') >= 0 && src.indexOf('fail-closed, Schema ≥۱') >= 0, 'مسیر fail-closed حساب Schema ≥۱ باید موجود باشد');
  const d = { schemaVersion:1, version:'1405.6.3α', warranties:[], invoices:[], sales:[], parts:[], products:[], inventory:{}, phonebook:[] };
  assertEqual(Object.prototype.hasOwnProperty.call(d,'accounts'), false, 'ورودی نباید accounts داشته باشد');
  const result = migrateBackup(d);
  assertEqual(Object.prototype.hasOwnProperty.call(result.data,'accounts'), false, 'migrateBackup نباید کلید accounts بسازد');
  assertEqual(result.data.accounts, undefined, 'مقدار accounts باید undefined بماند نه []');
  assertTrue((result.log||[]).some(function(x){ return /حساب/.test(x) && /غایب/.test(x) && /fail-closed/.test(x); }), 'لاگ باید غایب بودن حساب را بدون جایگزینی ثبت کند');
});

test('P1C-5 T11: مسیر v2.0 بدون accounts باید همچنان accounts=[] بسازد', () => {
  const oldBackup = { version:'2.0', invoices:[], products:[], inventory:{}, phonebook:[], invCtr:2, warranties:[] };
  const v = loadP1CValidator().validateRequiredBackupCollections(oldBackup);
  assertEqual(v.ok, true, 'validator Schema 0 بدون accounts باید PASS شود');
  assertTrue(v.missingRequiredCollections.indexOf('accounts')<0, 'accounts نباید در Schema 0 الزامی شود');
  const migrateBackup = loadMigrateBackupFn();
  const result = migrateBackup(JSON.parse(JSON.stringify(oldBackup)));
  assertArrayLength(result.data.accounts, 0, 'بک‌اپ قدیمی بدون accounts باید آرایه خالی accounts بگیرد، نه کرش کند');
  assertTrue((result.log||[]).some(function(x){ return /حساب/.test(x) && /نسخه قدیمی/.test(x); }), 'لاگ باید سازگاری نسخه قدیمی حساب را ثبت کند');
  const eng = loadBackupEngine(html);
  const originalJson = JSON.stringify({ version:'10.3.29', invoices:[{num:'1'}], phonebook:[], warranties:[{id:'W1'}] });
  const legacy = { version:'10.3.29', invoices:[{num:'1'}], phonebook:[], warranties:[{id:'W1'}] };
  const mig = eng.BackupEngine.migrateSchema(legacy, 1);
  assertTrue(mig.ok, 'Migration ۰→۱ باید موفق باشد');
  assertTrue(Array.isArray(mig.data.accounts) && mig.data.accounts.length===0, '۰→۱ باید accounts غایب Schema 0 را به [] تبدیل کند');
  assertEqual(JSON.parse(originalJson).accounts, undefined, 'فیکسچر اصلی نباید mutate شود');
  const r = p1c5RunRestoreGate('merge', { version:'2.0', warranties:[], invoices:[] }, [{id:'LIVE-ACC'}]);
  assertEqual(r.threw, null, 'merge Schema 0 بدون accounts نباید throw کند');
  assertEqual(r.liveUnchanged, true, 'ادغام بدون accounts نباید حساب زنده را پاک کند');
});

test('P1C-5 T12: گارانتی غایب همچنان FAIL است', () => {
  const v = loadP1CValidator().validateRequiredBackupCollections({schemaVersion:1, invoices:[], sales:[], parts:[], accounts:[]});
  assertEqual(v.ok, false, 'بدون warranties باید FAIL شود');
  assertTrue(v.missingRequiredCollections.indexOf('warranties')>=0, 'warranties باید missing بماند');
});

test('P1C-5 T13: فاکتور غایب همچنان FAIL است', () => {
  const v = loadP1CValidator().validateRequiredBackupCollections({schemaVersion:1, warranties:[], sales:[], parts:[], accounts:[]});
  assertEqual(v.ok, false, 'بدون invoices باید FAIL شود');
  assertTrue(v.missingRequiredCollections.indexOf('invoices')>=0, 'invoices باید missing بماند');
});

test('P1C-5 T14: حفاظت Schema فروش و قطعات باید بماند', () => {
  const v0 = loadP1CValidator().validateRequiredBackupCollections({version:'2.0', warranties:[], invoices:[]});
  assertEqual(v0.ok, true, 'Schema 0 بدون sales/parts/accounts باید PASS بماند');
  const missSales = loadP1CValidator().validateRequiredBackupCollections({schemaVersion:1, warranties:[], invoices:[], parts:[], accounts:[]});
  assertEqual(missSales.ok, false, 'Schema ≥۱ بدون sales باید FAIL بماند');
  assertTrue(missSales.missingRequiredCollections.indexOf('sales')>=0, 'sales باید missing بماند');
  const missParts = loadP1CValidator().validateRequiredBackupCollections({schemaVersion:1, warranties:[], invoices:[], sales:[], accounts:[]});
  assertEqual(missParts.ok, false, 'Schema ≥۱ بدون parts باید FAIL بماند');
  assertTrue(missParts.missingRequiredCollections.indexOf('parts')>=0, 'parts باید missing بماند');
});

test('P1C-5 T15: tasks در این بسته الزامی نشده است', () => {
  const v = loadP1CValidator().validateRequiredBackupCollections({schemaVersion:1, warranties:[], invoices:[], sales:[], parts:[], accounts:[]});
  assertEqual(v.ok, true, 'بدون tasks باید PASS شود');
  assertTrue(v.missingRequiredCollections.indexOf('tasks')<0, 'tasks نباید به رجیستری اضافه شده باشد');
  const migrateBackup = loadMigrateBackupFn();
...
console.log('');
console.log('📋 گروه: P1C-7 یکپارچگی قابل‌حمل پشتیبان (checksum / sectionChecksums)');

function p1c7Port(data){
  const v = loadP1CValidator();
  assertTrue(typeof v.validateBackupPortableIntegrity === 'function', 'validateBackupPortableIntegrity باید استخراج شود');
  return v.validateBackupPortableIntegrity(data);
}
function p1c7Schema1Base(){
  return { schemaVersion:1, warranties:[], invoices:[], sales:[], parts:[], accounts:[] };
}
function p1c7ChecksumApi(){
  const { webcrypto } = require('crypto');
  const rawAttach = extractFunctionSource(html, 'attachChecksum');
  const rawVerify = extractFunctionSource(html, 'verifyChecksum');
  assertTrue(!!rawAttach && !!rawVerify, 'توابع checksum استخراج نشد');
  const attachSrc = rawAttach.startsWith('async') ? rawAttach : 'async '+rawAttach;
  const verifySrc = rawVerify.startsWith('async') ? rawVerify : 'async '+rawVerify;
  const src = p1cValidatorSrc(html) + '\n' + attachSrc + '\n' + verifySrc;
  return new Function('window','crypto', src + '\n return { attachChecksum:attachChecksum, verifyChecksum:verifyChecksum, backupChecksumCanonicalString:backupChecksumCanonicalString, backupChecksumPayload:backupChecksumPayload, backupChecksumExcludedKey:backupChecksumExcludedKey, backupSectionHash:backupSectionHash, validateBackupPortableIntegrity:validateBackupPortableIntegrity, validateBackupSectionChecksums:validateBackupSectionChecksums, validateBackupStructuralIntegrity:validateBackupStructuralIntegrity };')({ crypto: webcrypto }, webcrypto);
}

test('P1C-7 T1: بدون checksum سازگار است (legacy / file://)', () => {
  const r = p1c7Port(p1c7Schema1Base());
  assertEqual(r.ok, true, 'نبود checksum باید PASS باشد');
  assertEqual(r.checksumSkipped, true, 'باید skipped باشد');
  assertEqual(r.hasBackupId, false, 'backupId در فرمت فعلی وجود ندارد');
});

test('P1C-7 T2: checksumAlgo=none سازگار است', () => {
  const r = p1c7Port(Object.assign(p1c7Schema1Base(), { checksum:'', checksumAlgo:'none' }));
  assertEqual(r.ok, true, 'none باید PASS باشد');
});

test('P1C-7 T3: SHA-256 بعد از pretty-print دیسک و parse دوباره تأیید می‌شود', async () => {
  const api = p1c7ChecksumApi();
  const data = Object.assign(p1c7Schema1Base(), { exportedAt:'2026-09-04T00:00:00.000Z', invoices:[{id:'I1', num:'1'}] });
  await api.attachChecksum(data);
  assertEqual(data.checksumAlgo, 'SHA-256', 'باید SHA-256 نوشته شود');
  assertTrue(!!data.checksum && data.checksum.length === 64, 'باید هش hex ۶۴ کاراکتر باشد');
  const disk = JSON.stringify(data, null, 2);
  const reopened = JSON.parse(disk);
  const v = await api.verifyChecksum(reopened);
  assertEqual(v.ok, true, 'پس از بازگشایی بایت pretty باید تأیید شود: '+v.msg);
  assertEqual(v.skipped, false, 'نباید skip شود');
});

test('P1C-7 T4: جهش داده با SHA-256 باید FAIL شود', async () => {
  const api = p1c7ChecksumApi();
  const data = Object.assign(p1c7Schema1Base(), { invoices:[{id:'I1'}] });
  await api.attachChecksum(data);
  const disk = JSON.stringify(data, null, 2);
  const reopened = JSON.parse(disk);
  reopened.invoices[0].id = 'TAMPERED';
  const v = await api.verifyChecksum(reopened);
  assertEqual(v.ok, false, 'جهش payload باید checksum را بشکند');
});

test('P1C-7 T5: جهش exportedAt هش را نمی‌شکند (از payload خارج است)', async () => {
  const api = p1c7ChecksumApi();
  const data = Object.assign(p1c7Schema1Base(), { exportedAt:'2026-01-01T00:00:00.000Z' });
  await api.attachChecksum(data);
  const reopened = JSON.parse(JSON.stringify(data, null, 2));
  reopened.exportedAt = '2099-12-31T00:00:00.000Z';
  const v = await api.verifyChecksum(reopened);
  assertEqual(v.ok, true, 'exportedAt از هش خارج است');
  assertTrue(api.backupChecksumExcludedKey('exportedAt') && api.backupChecksumExcludedKey('checksum') && api.backupChecksumExcludedKey('checksumAlgo'), 'کلیدهای مستثنی باید ثابت بمانند');
  assertEqual(Object.prototype.hasOwnProperty.call(api.backupChecksumPayload(reopened), 'exportedAt'), false, 'payload نباید exportedAt داشته باشد');
});

test('P1C-7 T6: فقط فاصله pretty-print هش را نمی‌شکند (هش روی بایت خام فایل نیست)', async () => {
  const api = p1c7ChecksumApi();
  const data = Object.assign(p1c7Schema1Base(), { invoices:[{id:'I1'}] });
  await api.attachChecksum(data);
  const compact = JSON.stringify(data);
  const pretty = JSON.stringify(data, null, 2);
  assertTrue(compact !== pretty, 'compact و pretty باید فرق داشته باشند');
  const v1 = await api.verifyChecksum(JSON.parse(compact));
  const v2 = await api.verifyChecksum(JSON.parse(pretty));
  assertEqual(v1.ok, true, 'compact parse باید تأیید شود');
  assertEqual(v2.ok, true, 'pretty parse باید تأیید شود');
});

test('P1C-7 T7: الگوریتم ناشناخته با checksum ادعایی باید FAIL شود', () => {
  const r = p1c7Port(Object.assign(p1c7Schema1Base(), { checksum:'abcd', checksumAlgo:'MD5' }));
  assertEqual(r.ok, false, 'الگوریتم ناشناخته نباید تأیید شود');
  assertEqual(r.status, 'INVALID', 'وضعیت باید INVALID باشد');
});

test('P1C-7 T8: sectionChecksums مطابق باید PASS شود', () => {
  const v = loadP1CValidator();
  const data = Object.assign(p1c7Schema1Base(), { warranties:[{id:'W1'}] });
  data.sectionChecksums = { warranties: v.backupSectionHash(data.warranties), invoices: v.backupSectionHash(data.invoices) };
  const r = v.validateBackupPortableIntegrity(data);
  assertEqual(r.ok, true, 'هش بخش مطابق باید PASS باشد: '+(r.errors&&r.errors[0]));
});

test('P1C-7 T9: ناهماهنگی sectionChecksums باید FAIL شود', () => {
  const v = loadP1CValidator();
  const data = Object.assign(p1c7Schema1Base(), {
    warranties:[{id:'W1'},{id:'W2'}],
    sectionChecksums: { warranties: v.backupSectionHash([{id:'W1'}]) }
  });
  const r = v.validateBackupPortableIntegrity(data);
  assertEqual(r.ok, false, 'mismatch بخش باید FAIL باشد');
  assertTrue((r.sectionChecksumMismatches||[]).indexOf('warranties')>=0, 'warranties باید mismatch باشد');
});

test('P1C-7 T10: نبود sectionChecksums سازگار است', () => {
  const r = p1c7Port(p1c7Schema1Base());
  assertEqual(r.ok, true, 'نبود sectionChecksums باید PASS باشد');
});

test('P1C-7 T11: شکست portable → صفر mutation زنده و بدون confirm', () => {
  const v = loadP1CValidator();
  const backup = Object.assign(p1c7Schema1Base(), {
    warranties:[{id:'IMP-W'}],
    sectionChecksums: { warranties: v.backupSectionHash([{id:'OTHER'}]) }
  });
  const rMerge = p1c6RunRestoreGate('merge', backup);
  assertTrue(!!rMerge.threw && /هش بخش/.test(rMerge.threw), 'merge باید به‌خاطر sectionChecksums throw کند');
  assertEqual(rMerge.liveUnchanged, true, 'ادغام نباید داده زنده را عوض کند');
  const rRep = p1c6RunRestoreGate('replace', JSON.parse(JSON.stringify(backup)));
  assertTrue(!!rRep.threw, 'replace باید مسدود شود');
  assertEqual(rRep.liveUnchanged, true, 'جایگزینی نباید داده زنده را عوض کند');
  ...
  assertTrue(importSrc.indexOf("if(!confirm('⚠️ هشدار سلامت فایل") < 0, 'mismatch checksum نباید confirm داشته باشد');
  assertContainsString(importSrc, 'یکپارچگی checksum نامعتبر', 'importData باید checksum را fail-closed کند');
});

test('P1C-7 T12: فیکسچر تاریخی بدون checksum و sectionChecksums سازگار می‌ماند', () => {
  const old = { version:'2.0', schemaVersion:0, warranties:[{id:'W1'}], invoices:[] };
  const r = p1c7Port(old);
  assertEqual(r.ok, true, 'legacy باید PASS باشد');
  assertEqual(loadP1CValidator().validateBackupStructuralIntegrity(old).ok, true, 'ساختاری P1C-6 باید بماند');
});

test('P1C-7 T13: رگرسیون warranties P1C-1', () => {
  const v = loadP1CValidator();
  assertEqual(v.validateBackupStructuralIntegrity({ invoices:[] }).ok, false, 'warranties غایب');
  assertEqual(v.validateBackupStructuralIntegrity({ warranties:[], invoices:[] }).ok, true, 'warranties []');
});

test('P1C-7 T14: رگرسیون invoices P1C-2', () => {
  const v = loadP1CValidator();
  assertEqual(v.validateBackupStructuralIntegrity({ warranties:[] }).ok, false, 'invoices غایب');
  assertEqual(v.validateBackupStructuralIntegrity({ warranties:[], invoices:[] }).ok, true, 'invoices []');
});

test('P1C-7 T15: رگرسیون sales P1C-3', () => {
  const v = loadP1CValidator();
  assertEqual(v.validateBackupStructuralIntegrity({ schemaVersion:0, warranties:[], invoices:[] }).ok, true, 'schema 0 بدون sales');
  assertEqual(v.validateBackupStructuralIntegrity({ schemaVersion:1, warranties:[], invoices:[], parts:[], accounts:[] }).ok, false, 'schema 1 بدون sales');
});

test('P1C-7 T16: رگرسیون parts P1C-4', () => {
  const v = loadP1CValidator();
  assertEqual(v.validateBackupStructuralIntegrity({ schemaVersion:1, warranties:[], invoices:[], sales:[], accounts:[] }).ok, false, 'schema 1 بدون parts');
  assertEqual(v.validateBackupStructuralIntegrity(p1c7Schema1Base()).ok, true, 'schema 1 با parts=[]');
});

test('P1C-7 T17: رگرسیون accounts P1C-5 و counts P1C-6', () => {
  const v = loadP1CValidator();
  assertEqual(v.validateBackupStructuralIntegrity({ schemaVersion:1, warranties:[], invoices:[], sales:[], parts:[] }).ok, false, 'schema 1 بدون accounts');
  const mismatch = Object.assign(p1c7Schema1Base(), { warranties:[{id:'W1'},{id:'W2'}], itemCounts:{ warranties:1, invoices:0, sales:0, parts:0, accounts:0 } });
  assertEqual(v.validateBackupStructuralIntegrity(mismatch).ok, false, 'P1C-6 count mismatch باید FAIL بماند');
});

test('P1C-7 T18: tasks required نیست و backupId اختراع نشد', () => {
  const v = loadP1CValidator();
  assertEqual(v.validateRequiredBackupCollections(p1c7Schema1Base()).ok, true, 'بدون tasks باید PASS باشد');
  assertTrue(html.indexOf("var REQUIRED_BACKUP_COLLECTIONS = ['warranties', 'invoices']") >= 0, 'رجیستری always-required نباید عوض شود');
  assertTrue(!/_buildFullBackupData[\s\S]{0,400}backupId\s*:/.test(html), 'صادرکننده نباید backupId بسازد');
  const migrateBackup = loadMigrateBackupFn();
  const old = migrateBackup({ version:'2.0', invoices:[], products:[], inventory:{}, phonebook:[], invCtr:2, warranties:[] });
...
function arch9cMakeLs(map) {
  ...
  const stats = { getItem: 0, setItem: 0, removeItem: 0, key: 0 };
  const ls = {
    stats: stats,
    getItem: function(k){ stats.getItem++; return Object.prototype.hasOwnProperty.call(store, k) ? store[k] : null; },
    setItem: function(k, v){ stats.setItem++; store[k] = String(v); },
    removeItem: function(k){ stats.removeItem++; delete store[k]; },
    key: function(i){ stats.key++; return Object.keys(store)[i] || null; },
...
function arch9cLiveAssembly() {
  ...
  const sandbox = {
    ...
    indexedDB: {
      open: function(){ idbStats.open++; return {}; }
    }
  };
  const ramBefore = JSON.stringify({
    invoices: invoices, products: products, warranties: warranties, phonebook: phonebook,
    parts: parts, sales: sales, accounts: accounts, inventory: inventory, services: services
  });
  const allSrc = [
...
test('ARCH-9C alias: _safeArr/_safeObj هنوز ارجاع زنده برمی‌گردانند', () => {
  const run = arch9cLiveAssembly();
  const helperSrc = extractFunctionSource(html, '_safeArr') + '\n' + extractFunctionSource(html, '_safeObj');
  const helper = new Function('ctx', 'with(ctx){ ' + helperSrc + '\nreturn { arr: _safeArr(invoices), obj: _safeObj(inventory), nested: _safeArr(invoices)[0] }; }')(run.sandbox);
  assertTrue(helper.arr === run.invoices, '_safeArr(invoices) باید همان آرایه RAM باشد');
  assertTrue(helper.obj === run.inventory, '_safeObj(inventory) باید همان شیء RAM باشد');
  assertTrue(helper.nested === run.invoices[0], 'رکورد داخل آرایه زنده باید همان ارجاع باشد');
});

test('ARCH-9C alias: helperهای services هنوز آرایه مشترک RAM هستند', () => {
  const run = arch9cLiveAssembly();
  const helperSrc = extractFunctionSource(html, '_safeArr');
  const helper = new Function('ctx', 'with(ctx){ ' + helperSrc + '\nreturn { services: _safeArr(services), svcs: _safeArr(services) }; }')(run.sandbox);
  assertTrue(helper.services === run.services, 'services helper باید RAM باشد');
  assertTrue(helper.svcs === run.services, 'svcs helper باید همان RAM services باشد');
  assertTrue(helper.services === helper.svcs, 'قبل از clone مرز helper مشترک است');
});
...
test('ARCH-9C G1: export/restore/helper بدون تغییر مسیر (SHA256 قفل)', () => {
...
test('ARCH-9C G2: _safeArr/_safeObj هنوز ارجاع زنده برمی‌گردانند', () => {
  ...
  assertContainsString(build, 'invoices: b.invoices', 'برش REQUIRED فاکتور باید از آداپتر بیاید');
  assertContainsString(build, 'invoices: _safeArr(invoices).length', 'itemCounts همچنان RAM زنده را می‌شمارد');
});
...
console.log('');
console.log('📋 گروه: ARCH-9D سخت‌سازی تولیدی clone-on-assemble');

test('ARCH-9D G1: clone فقط در return اسمبل است (یک‌بار)', () => {
  const build = extractFunctionSource(html, '_buildFullBackupData');
  assertEqual(arch9cSha256(build), ARCH9D_BUILD_SHA256, 'SHA256 اسمبل باید همان clone-on-return باشد');
  const matches = build.match(/return JSON\.parse\(JSON\.stringify\(data\)\);/g) || [];
  assertEqual(matches.length, 1, 'باید دقیقاً یک clone در return باشد');
  assertTrue(build.indexOf('JSON.parse(JSON.stringify(_safeArr') < 0, 'نباید روی helperها clone تکراری باشد');
  assertTrue(/return JSON\.parse\(JSON\.stringify\(data\)\);\s*}\s*$/.test(build), 'clone باید آخرین return باشد');
});

test('ARCH-9D: اسمبل تولیدی از RAM جداست و جهش آن RAM را عوض نمی‌کند', () => {
  const run = arch9cLiveAssembly();
  const a = run.assembly;
  assertTrue(a.invoices !== run.invoices, 'invoices جداست');
  assertTrue(a.products !== run.products, 'products جداست');
  assertTrue(a.warranties !== run.warranties, 'warranties جداست');
  assertTrue(a.parts !== run.parts, 'parts جداست');
  assertTrue(a.sales !== run.sales, 'sales جداست');
  assertTrue(a.accounts !== run.accounts, 'accounts جداست');
  assertTrue(a.phonebook !== run.phonebook, 'phonebook جداست');
  assertTrue(a.inventory !== run.inventory, 'inventory جداست');
  assertTrue(a.services !== run.services, 'services جداست');
  assertTrue(a.svcs !== run.services, 'svcs جداست');
  assertTrue(a.services !== a.svcs, 'services و svcs بعد از clone دو آرایه جدا هستند');
  assertTrue(a.invoices[0] !== run.invoices[0], 'رکورد فاکتور جداست');
  assertTrue(a.invoices[0].someNested !== run.invoices[0].someNested, 'nested جداست');
  assertTrue(a.attachmentsIndex !== undefined && a.attachmentsIndex !== run.invoices, 'attachmentsIndex جداست');

  a.invoices.push({ invoiceId: 'INVUID-PUSHED' });
  a.invoices[0].someNested.value = 'changed';
  a.products.push({ id: 'PRD-PUSHED' });
  a.warranties.push({ id: 'W-PUSHED' });
  a.parts.push({ id: 'PT-PUSHED' });
  a.sales.push({ id: 'SALE-PUSHED' });
  a.accounts.push({ id: 'ACC-PUSHED' });
  a.phonebook.push({ fn: 'جدید' });
  a.inventory['SKU-1'].qty = 99;
  a.inventory['SKU-NEW'] = { qty: 1 };
  a.attachmentsIndex.push({ id: 'forged' });
  a.services.push({ id: 'SVC-PUSHED' });
  a.svcs.push({ id: 'SVCS-PUSHED' });

  assertEqual(run.invoices.length, 1, 'RAM invoices');
  assertEqual(run.invoices[0].someNested.value, 'original-nested', 'RAM nested');
  assertEqual(run.products.length, 1, 'RAM products');
  assertEqual(run.warranties.length, 1, 'RAM warranties');
  assertEqual(run.parts.length, 1, 'RAM parts');
  assertEqual(run.sales.length, 1, 'RAM sales');
  assertEqual(run.accounts.length, 1, 'RAM accounts');
  assertEqual(run.phonebook.length, 1, 'RAM phonebook');
  assertEqual(run.inventory['SKU-1'].qty, 4, 'RAM inventory qty');
  assertTrue(run.inventory['SKU-NEW'] === undefined, 'RAM inventory key');
  assertEqual(run.services.length, 1, 'RAM services');
  assertEqual(run.invoices[0].docs.length, 1, 'RAM docs');
  assertEqual(run.invoices[0].docs[0].name, 'فاکتور.pdf', 'RAM attachment name');
});

test('ARCH-9D: برابری محتوای JSON با RAM (بدون هویت ارجاع)', () => {
  const run = arch9cLiveAssembly();
  const a = run.assembly;
  assertEqual(JSON.stringify(a.invoices), JSON.stringify(run.invoices), 'invoices content');
  assertEqual(JSON.stringify(a.products), JSON.stringify(run.products), 'products content');
  assertEqual(JSON.stringify(a.warranties), JSON.stringify(run.warranties), 'warranties content');
  assertEqual(JSON.stringify(a.phonebook), JSON.stringify(run.phonebook), 'phonebook content');
  assertEqual(JSON.stringify(a.parts), JSON.stringify(run.parts), 'parts content');
  assertEqual(JSON.stringify(a.sales), JSON.stringify(run.sales), 'sales content');
  assertEqual(JSON.stringify(a.accounts), JSON.stringify(run.accounts), 'accounts content');
  assertEqual(JSON.stringify(a.inventory), JSON.stringify(run.inventory), 'inventory content');
  assertEqual(JSON.stringify(a.services), JSON.stringify(run.services), 'services content');
  assertEqual(JSON.stringify(a.svcs), JSON.stringify(run.services), 'svcs content برابر services است');
  assertEqual(a.itemCounts.invoices, 1, 'itemCounts.invoices');
  assertEqual(a.itemCounts.tasks, 0, 'itemCounts.tasks');
  assertTrue(a.sections.indexOf('invoices') >= 0 && a.sections.length >= 32, 'sections');
  assertTrue(a.printCenter && a.printCenter.enabled === true, 'printCenter');
  assertTrue(Array.isArray(a.attachmentsIndex) && a.attachmentsIndex.length >= 1, 'attachmentsIndex');
  assertTrue(JSON.stringify(a).indexOf('علی') >= 0, 'Persian');
  assertEqual(JSON.stringify({
    invoices: run.invoices, products: run.products, warranties: run.warranties, phonebook: run.phonebook,
    parts: run.parts, sales: run.sales, accounts: run.accounts, inventory: run.inventory, services: run.services
  }), run.ramBefore, 'اسمبل نباید RAM را هنگام ساخت عوض کند');
});

test('ARCH-9D: اسمبل تولیدی نوشتن localStorage/IndexedDB ندارد', () => {
  const run = arch9cLiveAssembly();
  assertEqual(run.sandbox.localStorage.stats.setItem, 0, 'setItem باید ۰ باشد');
  assertEqual(run.sandbox.localStorage.stats.removeItem, 0, 'removeItem باید ۰ باشد');
  assertEqual(run.idbStats.open, 0, 'indexedDB.open باید ۰ باشد');
  const build = extractFunctionSource(html, '_buildFullBackupData');
  assertTrue(build.indexOf('localStorage.setItem') < 0, 'اسمبل نباید setItem صدا بزند');
  assertTrue(build.indexOf('indexedDB') < 0, 'اسمبل نباید indexedDB داشته باشد');
  assertTrue(build.indexOf('sv()') < 0 && build.indexOf('svWarr') < 0, 'اسمبل نباید persist زنده صدا بزند');
});

test('ARCH-9D: اعتبارسنج ساختاری HTML روی اسمبل تولیدی PASS است', () => {
  const run = arch9cLiveAssembly();
  const v = loadP1CValidator();
  const required = v.validateRequiredBackupCollections(run.assembly);
  assertTrue(required.ok, 'required collections: ' + JSON.stringify(required.errors || required));
  const structural = v.validateBackupStructuralIntegrity(run.assembly);
  assertTrue(structural.ok, 'structural: ' + JSON.stringify(structural.errors || structural));
  const counts = v.validateBackupItemCounts(run.assembly);
  assertTrue(counts.ok, 'itemCounts: ' + JSON.stringify(counts.errors || counts));
});

test('ARCH-9D: buildBackupObject همان محتوا را با هویت جدا می‌گیرد', () => {
  const run = arch9cLiveAssembly();
  const buildSrc = extractFunctionSource(html, 'buildBackupObject');
  const helperSrc = [
    extractFunctionSource(html, '_safeArr'),
  ...
  const obj = new Function('ctx', 'with(ctx){ ' + helperSrc + '\nreturn buildBackupObject(); }')(run.sandbox);
  assertEqual(obj.origin, 'autosave', 'origin autosave');
  assertEqual(obj.autoSave, true, 'autoSave');
  assertTrue(obj.invoices !== run.invoices, 'export path invoices جداست');
  assertEqual(JSON.stringify(obj.invoices), JSON.stringify(run.invoices), 'محتوای invoices');
  assertEqual(JSON.stringify(obj.phonebook), JSON.stringify(run.phonebook), 'محتوای phonebook');
  assertContainsString(extractFunctionSource(html, 'exportData'), 'var data = _buildFullBackupData();', 'exportData همچنان همان اسمبل را می‌گیرد');
});

test('ARCH-9D G2: Restore/Phonebook/SQLite/resetAll قفل منبع', () => {
  ['_safeArr', '_safeObj', 'applyBackupSelective', 'applyBackupMergeSections', 'applyBackupReplaceSections', 'importData', 'savePBContact', 'resetAll'].forEach(function(name){
    const src = extractFunctionSource(html, name);
    assertTrue(src !== null, name + ' باید موجود باشد');
    assertTrue(src.indexOf('return JSON.parse(JSON.stringify(data));') < 0, name + ' نباید clone اسمبل را تکرار کند');
  });
  const repo = fs.readFileSync(path.join(path.dirname(filePath), 'desktop', 'Sirman.Core', 'Data', 'Repositories', 'JsonBackupRepository.cs'), 'utf8');
  assertContainsString(repo, 'html-backup-engine', 'JsonBackupRepository باید TBD بماند');
});
...
test('ARCH-14 G1: مسیر export/autosave آداپتر را صدا نمی‌زنند', () => {
  const exp = extractFunctionSource(html, 'exportData');
  const buildObj = extractFunctionSource(html, 'buildBackupObject');
  const build = extractFunctionSource(html, '_buildFullBackupData');
  ...
  const calls = build.match(/collectBackupSettingsSnapshot\s*\(\s*\)/g) || [];
  assertEqual(calls.length, 1, 'اسمبل باید دقیقاً یک‌بار آداپتر را صدا بزند');
...
console.log('');
console.log('📋 گروه: ARCH-15 cutover برش تنظیمات داخل _buildFullBackupData');

const ARCH15_SETTINGS_ASSIGN_KEYS = ['appliedUpdates','updatePackages','printSettings','company','serviceCenter','starredAlarms','appearance','sms','tz','networkSettings','prefs','aiKeys'];

function arch15RunAssembler(lsMap) {
  const ram = {
    invoices: [], products: [], inventory: {}, invCtr: 1, invoiceUidCtr: 0, saleCtr: 1, saleUidCtr: 0,
    phonebook: [], parts: [], services: [], warranties: [], sales: [], tasks: [], accounts: [],
    defectiveStock: [], warehouseDocs: [], stockMoves: [], warehouses: [], daqi: [], daqiWarehouse: [],
...
function arch15AssertThrows(fn, msg) {
  let threw = false;
  try { fn(); } catch (e) { threw = true; }
  assertTrue(threw, msg);
}

test('ARCH-15 G1: آداپتر فقط داخل اسمبل و دقیقاً یک‌بار', () => {
  const build = extractFunctionSource(html, '_buildFullBackupData');
  const exp = extractFunctionSource(html, 'exportData');
  const buildObj = extractFunctionSource(html, 'buildBackupObject');
  const sha = arch9cSha256(build);
  assertTrue(sha !== ARCH9D_BUILD_SHA256_PRE_ARCH15, 'SHA اسمبل باید نسبت به ARCH-9D/14 عوض شده باشد');
  assertEqual(sha, ARCH9D_BUILD_SHA256, 'قفل SHA جدید اسمبل');
  const calls = build.match(/collectBackupSettingsSnapshot\s*\(\s*\)/g) || [];
  assertEqual(calls.length, 1, 'collectBackupSettingsSnapshot باید دقیقاً یک‌بار صدا شود');
  assertContainsString(build, 'var s = collectBackupSettingsSnapshot();', 'یک فراخوانی در ابتدای اسمبل');
  assertTrue(exp.indexOf('collectBackupSettingsSnapshot') < 0, 'exportData نباید آداپتر را صدا بزند');
  assertTrue(buildObj.indexOf('collectBackupSettingsSnapshot') < 0, 'buildBackupObject نباید آداپتر را صدا بزند');
  assertContainsString(exp, 'var data = _buildFullBackupData();', 'exportData همچنان اسمبل کامل است');
  assertContainsString(buildObj, 'var d = _buildFullBackupData();', 'autosave همچنان اسمبل کامل است');
  assertContainsString(build, 'return JSON.parse(JSON.stringify(data));', 'clone-on-assemble باید بماند');
  const clones = build.match(/return JSON\.parse\(JSON\.stringify\(data\)\);/g) || [];
  assertEqual(clones.length, 1, 'باید دقیقاً یک clone در return باشد');
  assertTrue(build.indexOf('localStorage.setItem') < 0, 'اسمبل نباید LS بنویسد');
  assertTrue(build.indexOf('indexedDB') < 0, 'اسمبل نباید IndexedDB داشته باشد');
  assertTrue(build.indexOf("localStorage.getItem('laegh_printSettings')") < 0, 'اسمبل نباید مستقیم printSettings بخواند');
  assertTrue(build.indexOf("localStorage.getItem('laegh_tz')") < 0, 'اسمبل نباید مستقیم tz بخواند');
  assertTrue(build.indexOf('getPrintCenterState') < 0, 'اسمبل نباید مستقیم getPrintCenterState صدا بزند');
  ARCH15_SETTINGS_ASSIGN_KEYS.forEach(function(k){
    const n = (build.match(new RegExp('\\b' + k + '\\s*:', 'g')) || []).length;
    assertEqual(n, 1, k + ' باید دقیقاً یک انتساب در اسمبل داشته باشد');
  });
  assertEqual((build.match(/data\.printCenter = s\.printCenter/g) || []).length, 1, 'printCenter باید یک انتساب از آداپتر باشد');
});

ARCH14_FIXTURES.cases.forEach(function(fx){
  test('ARCH-15 '+fx.id+' equivalence: JSON.stringify(oldSettings)===JSON.stringify(newSettings)', () => {
    const newSettings = arch14PickSettings(arch15RunAssembler(fx.ls));
    const adapter = arch14RunAdapter(fx.ls);
    assertEqual(JSON.stringify(newSettings), JSON.stringify(fx.expected), fx.id+' assembler vs golden');
    assertEqual(JSON.stringify(newSettings), JSON.stringify(adapter), fx.id+' assembler vs adapter');
  });
});

test('ARCH-15 N1: printSettings خراب همچنان throw می‌کند', () => {
  arch15AssertThrows(function(){ arch15RunAssembler({ laegh_printSettings: '{bad' }); }, 'malformed printSettings');
});

test('ARCH-15 N2: company خراب همچنان throw می‌کند', () => {
  arch15AssertThrows(function(){ arch15RunAssembler({ laegh_company: '{bad' }); }, 'malformed company');
});

test('ARCH-15 N3: serviceCenter خراب همچنان throw می‌کند', () => {
  arch15AssertThrows(function(){ arch15RunAssembler({ laegh_service_center: '{bad' }); }, 'malformed serviceCenter');
});

test('ARCH-15 N4: sms خراب همچنان throw می‌کند', () => {
  arch15AssertThrows(function(){ arch15RunAssembler({ laegh_sms: '{bad' }); }, 'malformed sms');
});

test('ARCH-15 N5: tz غایب → Asia/Tehran', () => {
  const t2 = ARCH14_FIXTURES.cases.find(function(c){ return c.id==='T2'; });
  const got = arch14PickSettings(arch15RunAssembler(t2.ls));
  assertEqual(got.tz, 'Asia/Tehran', 'default tz');
  assertEqual(JSON.stringify(got), JSON.stringify(t2.expected), 'T2 golden');
});

test('ARCH-15 N6: کلید ظاهر غایب → رشته خالی', () => {
  const t3 = ARCH14_FIXTURES.cases.find(function(c){ return c.id==='T3'; });
  const got = arch14PickSettings(arch15RunAssembler(t3.ls));
  Object.keys(got.appearance).forEach(function(k){
    assertEqual(got.appearance[k], '', 'appearance.'+k);
  });
  assertEqual(Object.keys(got.appearance).length, 24, '۲۴ کلید ظاهر');
  assertEqual(JSON.stringify(got), JSON.stringify(t3.expected), 'T3 golden');
});

test('ARCH-15 N7: printCenter اختیاری غایب از نظر معنایی عوض نشده', () => {
  const t18 = ARCH14_FIXTURES.cases.find(function(c){ return c.id==='T18'; });
  const got = arch14PickSettings(arch15RunAssembler(t18.ls));
  assertEqual(JSON.stringify(got.printCenter), JSON.stringify(t18.expected.printCenter), 'printCenter default');
  assertEqual(JSON.stringify(got), JSON.stringify(t18.expected), 'T18 golden');
});

test('ARCH-15: اسمبل هنگام ساخت localStorage/IndexedDB نمی‌نویسد', () => {
  const t1 = ARCH14_FIXTURES.cases.find(function(c){ return c.id==='T1'; });
  const ls = arch9cMakeLs(t1.ls);
  const ram = {
    invoices: [{id:'INV-1'}], products: [], inventory: {}, invCtr: 1, invoiceUidCtr: 0, saleCtr: 1, saleUidCtr: 0,
    phonebook: [], parts: [], services: [], warranties: [], sales: [], tasks: [], accounts: [],
    defectiveStock: [], warehouseDocs: [], stockMoves: [], warehouses: [], daqi: [], daqiWarehouse: [],
    daqiVouchers: [], postalHistory: [], userAuditLog: [], bgAuditLog: [], userRoles: [], loginPw: '',
    senderInfo: {}, logoSrc: '', acH: {}, SIRMAN_BACKUP_MAGIC: 'SIRMAN_BACKUP', SIRMAN_SCHEMA_VERSION: 1
  ...
  assertEqual(idbOpen, 0, 'indexedDB.open باید ۰ باشد');
  assertEqual(full.invoices[0].id, 'INV-1', 'RAM کسب‌وکار باید در اسمبل بماند');
  assertEqual(JSON.stringify(arch14PickSettings(full)), JSON.stringify(t1.expected), 'settings slice');
});

test('ARCH-15: Restore / Phonebook / Print / SQLite / Host بدون تغییر کارکردی', () => {
  ['applyBackupMergeSections','applyBackupReplaceSections','importData','savePBContact','resetAll','getPrintCenterState','getPrintSettings'].forEach(function(name){
    assertTrue(extractFunctionSource(html, name) !== null, name+' باید بماند');
  });
  assertEqual(arch9cSha256(extractFunctionSource(html, 'exportData')), ARCH9C_FN_SHA256.exportData, 'exportData SHA');
  assertEqual(arch9cSha256(extractFunctionSource(html, 'buildBackupObject')), ARCH9C_FN_SHA256.buildBackupObject, 'buildBackupObject SHA');
  assertEqual(arch9cSha256(extractFunctionSource(html, '_safeArr')), ARCH9C_FN_SHA256._safeArr, '_safeArr SHA');
  assertEqual(arch9cSha256(extractFunctionSource(html, '_safeObj')), ARCH9C_FN_SHA256._safeObj, '_safeObj SHA');
  const repo = fs.readFileSync(path.join(path.dirname(filePath), 'desktop', 'Sirman.Core', 'Data', 'Repositories', 'JsonBackupRepository.cs'), 'utf8');
  assertContainsString(repo, 'html-backup-engine', 'JsonBackupRepository باید TBD بماند');
  const sqlite = fs.readFileSync(path.join(path.dirname(filePath), 'desktop', 'Sirman.Persistence.Sqlite', 'Sirman.Persistence.Sqlite.csproj'), 'utf8');
  assertTrue(sqlite.length > 0, 'SQLite پروژه باید بماند');
...
test('ARCH-15: Sirman_Final.html و Laegh_Final.html بایت‌به‌بایت یکی هستند', () => {
  const sirman = fs.readFileSync(path.join(path.dirname(filePath), 'Sirman_Final.html'));
  const laeghPath = path.join(path.dirname(filePath), 'Laegh_Final.html');
  assertTrue(fs.existsSync(laeghPath), 'Laegh_Final.html باید موجود باشد');
  const laegh = fs.readFileSync(laeghPath);
...
test('ARCH-16: فهرست فیلدهای باقی‌مانده غیرتنظیمات قفل است', () => {
  ...
  ARCH16_REQUIRED.forEach(function(k){
    assertContainsString(build, k + ': b.' + k, k+' باید از آداپتر REQUIRED بیاید');
    assertTrue(build.indexOf(k + ': _safeArr(' + k + '),') < 0, k+' نباید انتساب مستقل _safeArr در سطح بالا داشته باشد');
  });
  assertContainsString(build, 'invCtr: b.counters.invCtr', 'invCtr از آداپتر');
  assertContainsString(build, 'invoiceUidCtr: b.counters.invoiceUidCtr', 'invoiceUidCtr از آداپتر');
  assertContainsString(build, 'saleCtr: b.counters.saleCtr', 'saleCtr از آداپتر');
  assertContainsString(build, 'saleUidCtr: b.counters.saleUidCtr', 'saleUidCtr از آداپتر');
...
console.log('');
console.log('📋 گروه: ARCH-17 آداپتر REQUIRED کسب‌وکار (بدون cutover)');

const ARCH17_REQUIRED_KEYS = ['invoices','sales','warranties','parts','accounts','counters'];
const ARCH17_COUNTER_KEYS = ['invCtr','invoiceUidCtr','saleCtr','saleUidCtr'];
const ARCH17_FIXTURES = JSON.parse(fs.readFileSync(path.join(path.dirname(filePath), 'desktop', 'Sirman.Core.Tests', 'BusinessDataFixtures.json'), 'utf8'));
const ARCH17_BANNED = ['phonebook','tasks','services','svcs','warehouseDocs','stockMoves','defectiveStock','attachmentsIndex','printSettings','localStorage','indexedDB','sirmanHost'];

function arch17HelperSrc() {
  return ['_safeArr','_safeObj','_safeStr','collectRequiredBusinessSnapshot']
    .map(function(n){ const s = extractFunctionSource(html, n); if(!s) throw new Error('fn '+n); return s; })
    .join('\n');
}

function arch17BaseRam(extra) {
  return Object.assign({
    invoices: [], sales: [], warranties: [], parts: [], accounts: [],
    invCtr: 0, invoiceUidCtr: 0, saleCtr: 0, saleUidCtr: 0,
    products: [], inventory: {}, phonebook: [], services: [], tasks: [],
...
function arch17RunAdapter(ram, extras) {
  extras = extras || {};
...
function arch17AssemblerAndAdapter(ram) {
...
test('ARCH-17 G1: آداپتر REQUIRED فقط داخل اسمبل و دقیقاً یک‌بار', () => {
  const build = extractFunctionSource(html, '_buildFullBackupData');
  const exp = extractFunctionSource(html, 'exportData');
  const buildObj = extractFunctionSource(html, 'buildBackupObject');
  ...
  assertContainsString(build, 'var b = collectRequiredBusinessSnapshot();', 'یک فراخوانی در ابتدای اسمبل');
  assertTrue(exp.indexOf('collectRequiredBusinessSnapshot') < 0, 'exportData نباید آداپتر REQUIRED را صدا بزند');
  assertTrue(buildObj.indexOf('collectRequiredBusinessSnapshot') < 0, 'buildBackupObject نباید آداپتر REQUIRED را صدا بزند');
  assertContainsString(exp, 'var data = _buildFullBackupData();', 'exportData همچنان اسمبل است');
  assertContainsString(buildObj, 'var d = _buildFullBackupData();', 'autosave همچنان اسمبل است');
});

test('ARCH-17 G2: آداپتر فقط RAM REQUIRED را می‌خواند و clone می‌کند', () => {
  const src = extractFunctionSource(html, 'collectRequiredBusinessSnapshot');
  assertTrue(src !== null, 'collectRequiredBusinessSnapshot باید موجود باشد');
  ARCH17_REQUIRED_KEYS.forEach(function(k){
    assertContainsString(src, k + ':', 'کلید '+k);
  });
  ['_safeArr(invoices)','_safeArr(sales)','_safeArr(warranties)','_safeArr(parts)','_safeArr(accounts)'].forEach(function(pat){
    assertContainsString(src, pat, pat);
  });
  assertContainsString(src, 'invCtr||1', 'invCtr همان اسمبل');
  assertContainsString(src, 'invoiceUidCtr>0', 'invoiceUidCtr همان اسمبل');
  assertContainsString(src, 'saleCtr>0', 'saleCtr همان اسمبل');
  assertContainsString(src, 'saleUidCtr>0', 'saleUidCtr همان اسمبل');
  assertContainsString(src, 'return JSON.parse(JSON.stringify(data));', 'clone نهایی');
  ARCH17_BANNED.forEach(function(tok){
    assertTrue(src.indexOf(tok) < 0, 'آداپتر نباید '+tok+' داشته باشد');
  });
  assertTrue(src.indexOf('localStorage.setItem') < 0, 'بدون نوشتن LS');
  assertTrue(src.indexOf('setItem') < 0, 'بدون setItem');
  assertTrue(src.indexOf('nextInvoiceId') < 0 && src.indexOf('ensureInvoiceIdentity') < 0, 'شناسه جدید نسازد');
});

ARCH17_FIXTURES.cases.forEach(function(fx){
  test('ARCH-17 '+fx.id+' '+fx.note+': خروجی آداپتر با فیکسچر طلایی یکی است', () => {
    const run = arch17RunAdapter(fx.ram);
...
test('ARCH-17: برش REQUIRED اسمبل با آداپتر یکی است (T1)', () => {
  const t1 = ARCH17_FIXTURES.cases.find(function(c){ return c.id==='T1'; });
  const both = arch17AssemblerAndAdapter(t1.ram);
  assertEqual(JSON.stringify(arch17PickRequired(both.full)), JSON.stringify(both.adapter), 'required slice');
  assertEqual(JSON.stringify(both.adapter), JSON.stringify(t1.expected), 'adapter vs golden');
...
ARCH17_FIXTURES.cases.forEach(function(fx){
  test('ARCH-17 '+fx.id+': برش اسمبل ≡ آداپتر', () => {
    const both = arch17AssemblerAndAdapter(fx.ram);
    assertEqual(JSON.stringify(arch17PickRequired(both.full)), JSON.stringify(both.adapter), fx.id);
  });
});

test('ARCH-17: جهش خروجی آداپتر RAM را عوض نمی‌کند', () => {
  const invoices = [{ invoiceId:'INVUID-000001', nested:{v:'live'}, items:[{code:'A'}] }];
  const sales = [{ saleUid:'SALEUID-000001', nested:{t:1} }];
  const warranties = [{ id:'W-1', nested:{s:'x'} }];
  const parts = [{ id:'P-1', nested:{bin:'A'} }];
  const accounts = [{ id:'ACC-1', nested:{bal:1}, transactions:[{amount:2}] }];
  const invOrder = invoices.map(function(r){ return r.invoiceId; }).join(',');
  const ls = arch9cMakeLs({});
  const extras = { ls: ls, hostCalled: false, idbStats: { open: 0 } };
  const run = arch17RunAdapter({
    invoices: invoices, sales: sales, warranties: warranties, parts: parts, accounts: accounts,
    invCtr: 3, invoiceUidCtr: 1, saleCtr: 2, saleUidCtr: 1
  }, extras);
  const got = run.got;
  assertTrue(got.invoices !== invoices, 'آرایه فاکتور جداست');
  assertTrue(got.sales !== sales, 'آرایه فروش جداست');
  assertTrue(got.warranties !== warranties, 'آرایه گارانتی جداست');
  assertTrue(got.parts !== parts, 'آرایه قطعات جداست');
  assertTrue(got.accounts !== accounts, 'آرایه حساب جداست');
  got.invoices.push({ invoiceId:'FORGED' });
  got.invoices[0].nested.v = 'mut';
  got.invoices[0].items[0].code = 'Z';
  got.sales[0].nested.t = 9;
  got.warranties[0].nested.s = 'mut';
  got.parts[0].nested.bin = 'X';
  got.accounts[0].nested.bal = 8;
  got.accounts[0].transactions[0].amount = 99;
  got.counters.invCtr = 0;
  got.counters.saleUidCtr = 99;
  assertEqual(invoices.length, 1, 'طول invoices منبع');
  assertEqual(sales.length, 1, 'طول sales منبع');
  assertEqual(warranties.length, 1, 'طول warranties منبع');
  assertEqual(parts.length, 1, 'طول parts منبع');
  assertEqual(accounts.length, 1, 'طول accounts منبع');
  assertEqual(invoices[0].nested.v, 'live', 'nested invoice');
  assertEqual(invoices[0].items[0].code, 'A', 'nested invoice item');
  assertEqual(sales[0].nested.t, 1, 'nested sale');
  assertEqual(warranties[0].nested.s, 'x', 'nested warranty');
  assertEqual(parts[0].nested.bin, 'A', 'nested part');
  assertEqual(accounts[0].nested.bal, 1, 'nested account');
  assertEqual(accounts[0].transactions[0].amount, 2, 'nested trx');
  assertEqual(run.ctx.invCtr, 3, 'invCtr منبع');
  assertEqual(run.ctx.saleUidCtr, 1, 'saleUidCtr منبع');
  assertEqual(invoices.map(function(r){ return r.invoiceId; }).join(','), invOrder, 'ترتیب منبع');
...
test('ARCH-17: فایروال Phonebook / Restore / Print / SQLite', () => {
  const src = extractFunctionSource(html, 'collectRequiredBusinessSnapshot');
  assertTrue(src.indexOf('phonebook') < 0, 'آداپتر phonebook نمی‌خواند');
  assertTrue(src.indexOf('savePBContact') < 0, 'آداپتر savePBContact ندارد');
  const savePB = extractFunctionSource(html, 'savePBContact');
...
test('ARCH-17: Sirman_Final.html و Laegh_Final.html بایت‌به‌بایت یکی هستند', () => {
  const sirman = fs.readFileSync(path.join(path.dirname(filePath), 'Sirman_Final.html'));
  const laeghPath = path.join(path.dirname(filePath), 'Laegh_Final.html');
  assertTrue(fs.existsSync(laeghPath), 'Laegh_Final.html باید موجود باشد');
  const laegh = fs.readFileSync(laeghPath);
...
console.log('');
console.log('📋 گروه: ARCH-19 مرز پیوست/ارجاع (فقط ممیزی، بدون cutover)');
...
function arch19HelperSrc() {
  return ['isDiskRef', 'collectAttachmentIndex', 'validateBackupAttachmentIndex']
    .map(function(n){ const s = extractFunctionSource(html, n); if(!s) throw new Error('fn '+n); return s; })
    .join('\n');
}

function arch19RunCollector(bag, extras) {
  extras = extras || {};
  const idbStats = extras.idbStats || { open: 0 };
  ...
  const got = new Function('ctx', 'with(ctx){ ' + arch19HelperSrc() + '\nreturn collectAttachmentIndex(ctx); }')(ctx);
  return { got: got, ctx: ctx, idbStats: idbStats, extras: extras, ls: ls };
}

function arch19Validate(d) {
  const ctx = { window: { DISK_REF_PREFIX: 'disk://' }, d: d };
  return new Function('ctx', 'with(ctx){ ' + arch19HelperSrc() + '\nreturn validateBackupAttachmentIndex(d); }')(ctx);
}

function arch19AssemblerIndex(ram) {
  const ls = arch9cMakeLs({
    laegh_printSettings: '{}',
    laegh_company: '{}',
    laegh_tz: 'Asia/Tehran'
...
test('ARCH-19 G1: قفل SHA اسمبل و ARCH-17/18 و Restore و collectAttachmentIndex', () => {
  const build = extractFunctionSource(html, '_buildFullBackupData');
  const exp = extractFunctionSource(html, 'exportData');
  const buildObj = extractFunctionSource(html, 'buildBackupObject');
  const req = extractFunctionSource(html, 'collectRequiredBusinessSnapshot');
  ...
  assertTrue(build.indexOf('collectAttachmentReferenceSnapshot') < 0, 'اسمبل آداپتر جدید ندارد');
  assertTrue(html.indexOf('function collectAttachmentReferenceSnapshot') < 0, 'آداپتر HTML جدید ساخته نشده');
  assertTrue(idx.indexOf('invoiceId') < 0, 'واکر invoiceId نمی‌خواند');
  assertTrue(idx.indexOf('saleUid') < 0, 'واکر saleUid نمی‌خواند');
  assertTrue(idx.indexOf('phonebook') < 0, 'واکر phonebook نمی‌خواند');
});

test('ARCH-19 G2: واکر فقط warranties/sales/invoices و rec.id', () => {
  const src = extractFunctionSource(html, 'collectAttachmentIndex');
  assertContainsString(src, "walk(d && d.warranties, 'warranty')", 'warranty');
  assertContainsString(src, "walk(d && d.sales, 'sale')", 'sale');
  assertContainsString(src, "walk(d && d.invoices, 'invoice')", 'invoice');
  assertContainsString(src, 'parentId: parentId || \'\'', 'parentId از rec.id');
  assertContainsString(src, 'pushDoc(kind, rec.id, doc, i)', 'object-map rec.id');
  assertContainsString(src, 'pushDoc(kind, rec && rec.id, doc, i)', 'array rec.id');
  assertTrue(src.indexOf('d.tasks') < 0, 'tasks واکر ندارد');
  assertTrue(src.indexOf('stockMoves') < 0, 'stockMoves جداست');
  assertTrue(src.indexOf('agencyPhonebookIdx') < 0, 'daqi جداست');
});

ARCH19_FIXTURES.cases.forEach(function(fx){
  test('ARCH-19 '+fx.id+' '+fx.note+': collectAttachmentIndex با فیکسچر طلایی یکی است', () => {
    const run = arch19RunCollector(fx.bag);
    assertEqual(JSON.stringify(run.got), JSON.stringify(fx.expectedIndex), fx.id);
    assertTrue(!run.got.some(function(row){ return row && row.kind==='phonebook'; }), fx.id+' بدون kind دفترچه');
    assertEqual(run.ls.stats.setItem, 0, fx.id+' بدون نوشتن LS');
    assertEqual(run.idbStats.open, 0, fx.id+' بدون IDB');
  });
});

test('ARCH-19: برش attachmentsIndex اسمبل با واکر یکی است (T2/T4/T8)', () => {
  ['T2','T4','T8'].forEach(function(id){
    const fx = ARCH19_FIXTURES.cases.find(function(c){ return c.id===id; });
    const both = arch19AssemblerIndex(fx.bag);
    assertEqual(JSON.stringify(both.full.attachmentsIndex), JSON.stringify(fx.expectedIndex), id+' assembler');
    assertEqual(JSON.stringify(both.index), JSON.stringify(fx.expectedIndex), id+' collector on RAM');
  });
});

test('ARCH-19 T3: parentId فاکتور rec.id است نه invoiceId/num', () => {
  const t3 = ARCH19_FIXTURES.cases.find(function(c){ return c.id==='T3'; });
  const row = arch19RunCollector(t3.bag).got[0];
  assertEqual(row.parentId, 'mig_inv_0_1', 'rec.id');
  assertTrue(row.parentId !== t3.bag.invoices[0].invoiceId, 'نه invoiceId');
  assertTrue(row.parentId !== t3.bag.invoices[0].num, 'نه num');
});

test('ARCH-19 T4: parentId فروش rec.id است نه saleUid', () => {
  const t4 = ARCH19_FIXTURES.cases.find(function(c){ return c.id==='T4'; });
  const row = arch19RunCollector(t4.bag).got[0];
  assertEqual(row.parentId, 'SL-0001', 'display id');
  assertTrue(row.parentId !== t4.bag.sales[0].saleUid, 'نه saleUid');
});

test('ARCH-19 T6: ارجاع یتیم در اعتبارسنج FAIL است و تعمیر نمی‌شود', () => {
  const t6 = ARCH19_FIXTURES.cases.find(function(c){ return c.id==='T6'; });
  const v = arch19Validate(t6.validatorInput);
  assertEqual(v.ok, false, 'orphan INVALID');
  assertTrue(v.brokenAttachmentRefs.some(function(b){ return b.parentId==='W-MISSING'; }), 'W-MISSING');
  assertEqual(JSON.stringify(t6.validatorInput.attachmentsIndex[0].parentId), JSON.stringify('W-MISSING'), 'بدون repair');
});

test('ARCH-19 T7/T11: kind نامعتبر و Phonebook ایندکس نمی‌شوند', () => {
  const t7 = ARCH19_FIXTURES.cases.find(function(c){ return c.id==='T7'; });
  const t11 = ARCH19_FIXTURES.cases.find(function(c){ return c.id==='T11'; });
  assertEqual(arch19RunCollector(t7.bag).got.length, 0, 'T7 empty');
  assertEqual(arch19RunCollector(t11.bag).got.length, 0, 'T11 empty');
  const v = arch19Validate(t7.validatorInput);
  assertEqual(v.ok, true, 'unsupported kind skipped');
});

test('ARCH-19 T8: فاکتور بدون rec.id → parentId خالی؛ اعتبارسنج رد نمی‌کند', () => {
  const t8 = ARCH19_FIXTURES.cases.find(function(c){ return c.id==='T8'; });
  const row = arch19RunCollector(t8.bag).got[0];
  assertEqual(row.parentId, '', 'empty parentId');
  assertEqual(arch19Validate(t8.validatorInput).ok, true, 'empty parentId compatible skip');
});

test('ARCH-19: سیستم‌های ارجاع مستقل قاطی نمی‌شوند', () => {
  const src = extractFunctionSource(html, 'collectAttachmentIndex');
  assertTrue(src.indexOf('refDoc') < 0, 'stockMoves.refDoc جداست');
  assertTrue(src.indexOf('agencyPhonebookIdx') < 0, 'daqi جداست');
  const bag = {
    warranties: [], sales: [], invoices: [],
    stockMoves: [{ id:'SM-1', refDoc:'W-1' }],
    daqi: [{ id:'DQ-1', agencyPhonebookIdx: 3 }]
  };
  assertEqual(JSON.stringify(arch19RunCollector(bag).got), '[]', 'ایندکس خالی');
});

test('ARCH-19: جهش خروجی واکر docs منبع را عوض نمی‌کند', () => {
  const docs = [{ name:'a.pdf', data:'disk://sirman_media/docs/a.pdf', nested:{v:'live'} }];
  const warranties = [{ id:'W-1', docs: docs }];
  const order = docs.map(function(d){ return d.name; }).join(',');
  const extras = { hostCalled: false, idbStats: { open: 0 } };
  const run = arch19RunCollector({ warranties: warranties, sales: [], invoices: [] }, extras);
  run.got[0].name = 'MUT';
  run.got.push({ id:'FORGED' });
  assertEqual(docs.length, 1, 'طول docs');
  assertEqual(docs[0].name, 'a.pdf', 'نام منبع');
  assertEqual(docs[0].nested.v, 'live', 'nested');
  assertEqual(docs.map(function(d){ return d.name; }).join(','), order, 'ترتیب');
  assertEqual(run.ls.stats.setItem, 0, 'بدون LS');
  assertEqual(extras.idbStats.open, 0, 'بدون IDB');
  assertTrue(!extras.hostCalled, 'بدون Host');
});

test('ARCH-19: فایروال Restore / Phonebook / Print / SQLite / نسخه', () => {
  const savePB = extractFunctionSource(html, 'savePBContact');
  assertTrue(savePB.indexOf("id:'PB-") < 0, 'savePBContact شناسه نمی‌سازد');
  ['applyBackupMergeSections','applyBackupReplaceSections','importData','savePBContact','resetAll','getPrintCenterState','collectRequiredBusinessSnapshot','collectOptionalBusinessSnapshot'].forEach(function(name){
    assertTrue(extractFunctionSource(html, name) !== null, name+' باید بماند');
  });
...
test('ARCH-19: Sirman_Final.html و Laegh_Final.html بایت‌به‌بایت یکی هستند', () => {
  const sirman = fs.readFileSync(path.join(path.dirname(filePath), 'Sirman_Final.html'));
  const laegh = fs.readFileSync(path.join(path.dirname(filePath), 'Laegh_Final.html'));
  assertEqual(sirman.length, laegh.length, 'طول فایل');
  assertEqual(Buffer.compare(sirman, laegh), 0, 'byte-identical');
...
console.log('');
console.log('📋 گروه: ARCH-20 cutover برش REQUIRED کسب‌وکار داخل _buildFullBackupData');

const ARCH20_REQUIRED_TOP_KEYS = ['invoices','sales','warranties','parts','accounts'];
const ARCH20_COUNTER_TOP_KEYS = ['invCtr','invoiceUidCtr','saleCtr','saleUidCtr'];
const ARCH20_OLD_ASSEMBLER_SHA256 = '17f08840ecb3e6ecc9d72082d27eeeb6736daa97a1f06819df4f4f04a998cfa6';
...
function arch20RunAssembler(ram, extras) {
  ...
  const full = new Function('ctx', 'with(ctx){ ' + arch20AssemblerRuntimeSrc() + '\nreturn _buildFullBackupData(); }')(ctx);
  return { full: full, ctx: ctx, ls: ls, idbStats: idbStats, extras: extras };
}

function arch20RunOldRequired(ram) {
...
function arch20PickRequiredTop(full) {
  return {
    invoices: full.invoices,
    sales: full.sales,
    warranties: full.warranties,
...
function arch20OldRequiredToTop(slice) {
  return {
    invoices: slice.invoices,
    sales: slice.sales,
    warranties: slice.warranties,
    parts: slice.parts,
    accounts: slice.accounts,
    invCtr: slice.counters.invCtr,
    invoiceUidCtr: slice.counters.invoiceUidCtr,
    saleCtr: slice.counters.saleCtr,
    saleUidCtr: slice.counters.saleUidCtr
  };
}

test('ARCH-20 G1: آداپتر REQUIRED فقط داخل اسمبل و دقیقاً یک‌بار؛ SHA عوض شده', () => {
  const build = extractFunctionSource(html, '_buildFullBackupData');
  const exp = extractFunctionSource(html, 'exportData');
  const buildObj = extractFunctionSource(html, 'buildBackupObject');
  const req = extractFunctionSource(html, 'collectRequiredBusinessSnapshot');
  ...
  assertTrue(exp.indexOf('collectRequiredBusinessSnapshot') < 0, 'exportData نباید آداپتر را صدا بزند');
  assertTrue(buildObj.indexOf('collectRequiredBusinessSnapshot') < 0, 'buildBackupObject نباید آداپتر را صدا بزند');
  assertEqual(arch9cSha256(req), ARCH17_REQUIRED_ADAPTER_SHA256, 'بدنه آداپتر ARCH-17 نباید عوض شود');
  assertEqual(arch9cSha256(opt), ARCH18_OPTIONAL_ADAPTER_SHA256, 'آداپتر ARCH-18 نباید عوض شود');
  assertEqual(arch9cSha256(idx), ARCH19_COLLECT_ATTACHMENT_INDEX_SHA256, 'collectAttachmentIndex نباید عوض شود');
  assertEqual(arch9cSha256(exp), ARCH9C_FN_SHA256.exportData, 'exportData SHA');
  assertEqual(arch9cSha256(buildObj), ARCH9C_FN_SHA256.buildBackupObject, 'buildBackupObject SHA');
  assertTrue(build.indexOf('localStorage.setItem') < 0, 'بدون نوشتن LS');
  assertTrue(build.indexOf('indexedDB') < 0, 'بدون IDB');
  assertTrue(/\bcounters\s*:/.test(build) === false, 'نباید کلید تو در توی counters در اسمبل باشد');
});

test('ARCH-20 G2: فقط شش فیلد REQUIRED از b؛ ترتیب کلید حفظ شده', () => {
  const build = extractFunctionSource(html, '_buildFullBackupData');
  assertContainsString(build, 'invoices: b.invoices', 'invoices');
  assertContainsString(build, 'sales: b.sales', 'sales');
  assertContainsString(build, 'warranties: b.warranties', 'warranties');
  assertContainsString(build, 'parts: b.parts', 'parts');
  assertContainsString(build, 'accounts: b.accounts', 'accounts');
  assertContainsString(build, 'invCtr: b.counters.invCtr', 'invCtr');
  assertContainsString(build, 'invoiceUidCtr: b.counters.invoiceUidCtr', 'invoiceUidCtr');
  assertContainsString(build, 'saleCtr: b.counters.saleCtr', 'saleCtr');
  assertContainsString(build, 'saleUidCtr: b.counters.saleUidCtr', 'saleUidCtr');
  ARCH20_REQUIRED_TOP_KEYS.forEach(function(k){
    const assigns = build.match(new RegExp('\\b' + k + '\\s*:\\s*b\\.' + k, 'g')) || [];
    assertEqual(assigns.length, 1, k+' باید یک انتساب از b داشته باشد');
    assertTrue(build.indexOf(k + ': _safeArr(' + k + '),') < 0, k+' نباید انتساب مستقل سطح بالا داشته باشد');
  });
  ARCH20_COUNTER_TOP_KEYS.forEach(function(k){
    const assigns = build.match(new RegExp('\\b' + k + '\\s*:\\s*b\\.counters\\.' + k, 'g')) || [];
  ...
  const warAt = build.indexOf('warranties: b.warranties');
  const salesAt = build.indexOf('sales: b.sales');
  const accAt = build.indexOf('accounts: b.accounts');
  assertTrue(invAt < prodAt && prodAt < invCtrAt && invCtrAt < pbAt && pbAt < partsAt, 'ترتیب invoices/products/invCtr/phonebook/parts');
  assertTrue(partsAt < warAt && warAt < salesAt && salesAt < accAt, 'ترتیب parts/warranties/sales/accounts');
});

ARCH17_FIXTURES.cases.forEach(function(fx){
  test('ARCH-20 '+fx.id+' equivalence: JSON.stringify(oldRequired)===JSON.stringify(newRequired)', () => {
    const oldSlice = arch20RunOldRequired(fx.ram);
    const run = arch20RunAssembler(fx.ram);
    const newNested = arch17PickRequired(run.full);
    const newTop = arch20PickRequiredTop(run.full);
    assertEqual(JSON.stringify(oldSlice), JSON.stringify(fx.expected), fx.id+' old vs golden');
    assertEqual(JSON.stringify(newNested), JSON.stringify(fx.expected), fx.id+' new nested vs golden');
    assertEqual(JSON.stringify(newNested), JSON.stringify(oldSlice), fx.id+' nested exact');
    assertEqual(JSON.stringify(newTop), JSON.stringify(arch20OldRequiredToTop(oldSlice)), fx.id+' top-level exact');
    assertTrue(!('counters' in run.full), fx.id+' بدون کلید counters تو در تو در بک‌آپ');
    assertTrue(Array.isArray(run.full.phonebook), fx.id+' phonebook در اسمبل مانده');
  });
});

test('ARCH-20: شیء کامل بک‌آپ — settings/required/optional/attachmentsIndex/envelope', () => {
  const t1 = ARCH17_FIXTURES.cases.find(function(c){ return c.id==='T1'; });
  const ram = Object.assign({}, t1.ram, {
    phonebook: [{ fn:'علی', phones:['۰۹۱۲'] }],
    products: [{ code:'P-1', nested:{v:'keep'} }],
    inventory: { 'P-1': { qty:2 } },
    services: [{ code:'SVC-1' }],
    tasks: [{ id:'TSK-1' }],
    defectiveStock: [{ id:'DF-1' }],
    warehouseDocs: [],
    stockMoves: [],
  ...
  const run = arch20RunAssembler(ram, { ls: ls });
  const full = run.full;
  const oldReq = arch20RunOldRequired(ram);
  const settingsExp = arch14RunAdapter({
    laegh_printSettings: JSON.stringify({ paper:'A4' }),
    laegh_company: JSON.stringify({ name:'لایق الکترونیک پارسیان' }),
    laegh_tz: 'Asia/Tehran'
  });
  assertEqual(JSON.stringify(arch17PickRequired(full)), JSON.stringify(oldReq), 'required vs old');
  assertEqual(JSON.stringify(arch14PickSettings(full)), JSON.stringify(settingsExp), 'settings ARCH-15');
  const opt = arch18RunAdapter(ram).got;
  assertEqual(JSON.stringify(arch18PickOptional(full)), JSON.stringify(opt), 'optional ARCH-18 firewall');
  assertEqual(JSON.stringify(full.phonebook), JSON.stringify([{ fn:'علی', phones:['۰۹۱۲'] }]), 'phonebook');
...
test('ARCH-20: ۱۲ مورد لبه — خالی/تو در تو/تکرار/ترتیب/null/یونیکد/بدون تعمیر', () => {
  const t2 = ARCH17_FIXTURES.cases.find(function(c){ return c.id==='T2'; });
  const t4 = ARCH17_FIXTURES.cases.find(function(c){ return c.id==='T4'; });
  const t5 = ARCH17_FIXTURES.cases.find(function(c){ return c.id==='T5'; });
  const t6 = ARCH17_FIXTURES.cases.find(function(c){ return c.id==='T6'; });
  const t8 = ARCH17_FIXTURES.cases.find(function(c){ return c.id==='T8'; });
  const t9 = ARCH17_FIXTURES.cases.find(function(c){ return c.id==='T9'; });
  const t10 = ARCH17_FIXTURES.cases.find(function(c){ return c.id==='T10'; });
  const empty = arch20RunAssembler(t2.ram).full;
  assertEqual(empty.invoices.length, 0, '1 empty arrays');
  assertEqual(JSON.stringify(arch20PickRequiredTop(empty)), JSON.stringify(arch20OldRequiredToTop(arch20RunOldRequired(t2.ram))), '2 empty counters');
  const nested = arch20RunAssembler(t4.ram).full;
  assertEqual(JSON.stringify(arch17PickRequired(nested)), JSON.stringify(t4.expected), '3 nested');
  const dups = arch20RunAssembler(t8.ram).full;
  assertEqual(dups.invoices.length, t8.ram.invoices.length, '4 duplicates remain');
  assertEqual(JSON.stringify(dups.invoices), JSON.stringify(t8.expected.invoices), '4 duplicate bytes');
  const ordered = arch20RunAssembler(t10.ram).full;
  assertEqual(JSON.stringify(ordered.invoices.map(function(r){ return r.invoiceId || r.num; })), JSON.stringify(t10.expected.invoices.map(function(r){ return r.invoiceId || r.num; })), '5 order');
  const nullRam = Object.assign({}, t2.ram, { invoices: [{ invoiceId:'INVUID-000001', note:null, extra:undefined }] });
  const withNull = arch20RunAssembler(nullRam).full;
  assertEqual(withNull.invoices[0].note, null, '6 null preserved');
  assertTrue(!('extra' in withNull.invoices[0]), '6 undefined dropped by JSON clone same as before');
  const persian = arch20RunAssembler(t5.ram).full;
  assertEqual(JSON.stringify(arch17PickRequired(persian)), JSON.stringify(t5.expected), '7 Persian Unicode');
  const missing = arch20RunAssembler(t9.ram).full;
  assertEqual(JSON.stringify(arch17PickRequired(missing)), JSON.stringify(t9.expected), '8 missing identity');
  const build = extractFunctionSource(html, '_buildFullBackupData');
  assertTrue(build.indexOf('nextInvoiceId') < 0 && build.indexOf('ensureInvoiceIdentity') < 0, '9 no generated IDs');
  const nums = arch20RunAssembler(t6.ram).full;
  assertEqual(nums.parts[0].qty, 0, '10 no normalize qty');
  assertEqual(nums.parts[0].price, 1.5, '10 no normalize float');
  assertEqual(nums.invoices[0].flag, '0', '10 string 0 stays string');
  assertEqual(dups.invoices.length, t8.expected.invoices.length, '11 no dedup');
  assertEqual(JSON.stringify(missing.invoices), JSON.stringify(t9.expected.invoices), '12 no repair');
});

test('ARCH-20: جهش قبل از clone نهایی از طریق نتیجه آداپتر RAM را عوض نمی‌کند', () => {
  const invoices = [{ invoiceId:'INVUID-000001', nested:{v:'live'}, items:[{code:'A'}] }];
  const sales = [{ saleUid:'SALEUID-000001', nested:{t:1} }];
  const warranties = [{ id:'W-1', nested:{s:'x'} }];
  const parts = [{ id:'P-1', nested:{bin:'A'} }];
  const accounts = [{ id:'ACC-1', nested:{bal:1}, transactions:[{amount:2}] }];
  const order = invoices.map(function(r){ return r.invoiceId; }).join(',');
  ...
  const src = arch14HelperSrc() + '\n' + arch17HelperSrc() + '\n' + extractFunctionSource(html, 'collectAttachmentIndex') + `
    ...
    data.invoices.push({ invoiceId:'FORGED' });
    data.invoices[0].nested.v = 'mut';
    data.invoices[0].items[0].code = 'Z';
    data.sales[0].nested.t = 9;
    data.warranties[0].nested.s = 'mut';
    data.parts[0].nested.bin = 'X';
    data.accounts[0].nested.bal = 8;
    data.accounts[0].transactions[0].amount = 99;
    data.invCtr = 0;
    data.saleUidCtr = 99;
    return {
      dataLen: data.invoices.length,
      ramLen: invoices.length,
      ramNested: invoices[0].nested.v,
      ramItem: invoices[0].items[0].code,
      ramSale: sales[0].nested.t,
      ramWar: warranties[0].nested.s,
      ramPart: parts[0].nested.bin,
      ramAcc: accounts[0].nested.bal,
      ramTrx: accounts[0].transactions[0].amount,
      ramInvCtr: invCtr,
      ramSaleUidCtr: saleUidCtr,
      ramOrder: invoices.map(function(r){ return r.invoiceId; }).join(',')
    };
  `;
  const probe = new Function('ctx', 'with(ctx){ ' + src + ' }')(ctx);
  assertEqual(probe.dataLen, 2, 'جهش data قبل از clone');
  assertEqual(probe.ramLen, 1, 'طول منبع invoices');
  assertEqual(probe.ramNested, 'live', 'nested invoice');
  assertEqual(probe.ramItem, 'A', 'invoice item');
  assertEqual(probe.ramSale, 1, 'nested sale');
  assertEqual(probe.ramWar, 'x', 'nested warranty');
  assertEqual(probe.ramPart, 'A', 'nested part');
  assertEqual(probe.ramAcc, 1, 'nested account');
  assertEqual(probe.ramTrx, 2, 'nested trx');
  assertEqual(probe.ramInvCtr, 3, 'invCtr منبع');
  assertEqual(probe.ramSaleUidCtr, 1, 'saleUidCtr منبع');
  assertEqual(probe.ramOrder, order, 'ترتیب منبع');
  const run = arch20RunAssembler({
    invoices: invoices, sales: sales, warranties: warranties, parts: parts, accounts: accounts,
    invCtr: 3, invoiceUidCtr: 1, saleCtr: 2, saleUidCtr: 1
  }, extras);
  run.full.invoices.push({ invoiceId:'AFTER' });
  run.full.invoices[0].nested.v = 'after';
  assertEqual(invoices.length, 1, 'خروجی نهایی هم RAM را عوض نمی‌کند');
  assertEqual(invoices[0].nested.v, 'live', 'nested پس از خروجی');
...
test('ARCH-20: فایروال optional / attachmentsIndex / Phonebook / Restore / Print', () => {
  ...
  const savePB = extractFunctionSource(html, 'savePBContact');
  assertTrue(savePB.indexOf("id:'PB-") < 0, 'savePBContact شناسه نمی‌سازد');
  ['applyBackupMergeSections','applyBackupReplaceSections','importData','savePBContact','resetAll','getPrintCenterState','getPrintSettings','collectOptionalBusinessSnapshot','collectAttachmentIndex'].forEach(function(name){
    assertTrue(extractFunctionSource(html, name) !== null, name+' باید بماند');
  });
...
test('ARCH-20: اسمبل هنگام ساخت localStorage/IndexedDB نمی‌نویسد', () => {
  const t1 = ARCH17_FIXTURES.cases.find(function(c){ return c.id==='T1'; });
  const extras = { hostCalled: false, idbStats: { open: 0 }, ls: arch9cMakeLs({
    laegh_printSettings: '{}',
    laegh_company: '{}',
  ...
  assertEqual(run.full.invoices[0].invoiceId, 'INVUID-000007', 'محتوا');
});
...
function networkPullPreviewBundle(srcHtml) {
  ...
  return p1cValidatorSrc(srcHtml) + '\n' + names.map(n => extractFunctionSource(srcHtml, n)).filter(Boolean).join('\n');
}
...
test('آماده‌سازی و انصراف نباید دادهٔ کسب‌وکار را عوض کنند', () => {
  ...
  const runner = new Function(bundle + `
    ...
    var pkg = {schemaVersion:1, origin:'network-workspace', invoices:[{invoiceId:'INV-2', customer:'تازه'}], products:[{code:'P2'}], warranties:[], sales:[], parts:[], accounts:[]};
...
test('حواله EXE نباید ترتیب جهش/خواندن را عوض کند و حرکت adjust بدون qty عددی نسازد', () => {
  ...
  const beforeApply = whSrc.slice(0, applyIdx);
  const afterApply = whSrc.slice(applyIdx);
  assertTrue(beforeApply.indexOf('invStockSnapshot') >= 0, 'پیش‌پرواز موجودی باید قبل از applyWarehouseDoc باشد');
  assertTrue(beforeApply.indexOf('stockDataAvailable') >= 0, 'پیش‌پرواز باید stockDataAvailable را قبل از جهش سند چک کند');
  assertTrue(afterApply.indexOf('invStockSnapshot') >= 0, 'خواندن موجودی adjust باید بعد از جهش سند بماند');
  assertTrue(afterApply.indexOf('stockDataAvailable') >= 0, 'گارد NaN باید بعد از جهش سند بماند نه به‌جای آن');
...
console.log('');
console.log('📋 گروه: فاز ۳ B19R بستن ریسک مرز جهش');

function makeExeWarehouseDoc(runImpl, type) {
  const runSrc = extractFunctionSource(html, 'runBusinessCore');
  const takeSrc = extractFunctionSource(html, 'takeBusinessCore');
  const hasSrc = extractFunctionSource(html, 'hasBusinessCore');
  const availSrc = extractFunctionSource(html, 'stockDataAvailable');
  ...
  const findSrc = extractFunctionSource(html, 'invFindStockItem');
  const moveSrc = extractFunctionSource(html, 'recordStockMove');
  const saveSrc = extractFunctionSource(html, 'saveWarehouseDoc');
  assertTrue(!!saveSrc && !!snapSrc && !!availSrc, 'saveWarehouseDoc / snapshot پیدا نشد');
  return new Function('runImpl', 'docType', runSrc + '\n' + takeSrc + '\n' + hasSrc + '\n' + availSrc + '\n' + (sumSrc || '') + '\n' + recSrc + '\n' + snapSrc + '\n' + findSrc + '\n' + moveSrc + '\n' + saveSrc + `
    var calls = [];
    var stockMoves = [];
    var ntfCalls = [];
    var warehouseDocs = [];
    var _whItems = [{code:'P1', name:'قطعه', qty:2, qtyPhy:2}];
    var parts = [{code:'P1', name:'قطعه', qty:10, min:0, reorder:0, reserved:0, byWh:{'WH-A':10}, reservedByWh:{}, price:0}];
    var inventory = {};
    var fields = {
      'wh-doc-type': {value: docType},
      'wh-party': {value:'طرف'},
      'wh-date': {value:'1405/05/30'},
      'wh-location': {value:''},
      'wh-location-out': {value:''},
      'wh-note': {value:''},
      'wh-reason': {value:'علت تست'},
      'wh-docno': {value:''},
      'wh-price': {value:'0'},
      'wh-fromwh': {value:'WH-A'},
      'wh-towh': {value:'WH-A'},
      'wh-same-company': {value:'', checked:false}
    };
    ...
    function auditUser(){}
    function emit(){}
    function persistCoreSnapshot(){}
    function renderWarehouseDocs(){}
    function checkStarredAlarms(){}
    function currentWhCatalog(){ return ''; }
    function getDefaultWhId(){ return 'WH-A'; }
    function fdt(){ return '1405/05/30'; }
    function invCurrentUser(){ return 'tester'; }
    function _whTag(){ return 'parts'; }
    ...
    saveWarehouseDoc();
    return {calls:calls, stockMoves:stockMoves, ntfCalls:ntfCalls, docs:warehouseDocs, parts:parts};
  `)(runImpl, type);
}

function makeExeRestock(runImpl) {
  const runSrc = extractFunctionSource(html, 'runBusinessCore');
  const takeSrc = extractFunctionSource(html, 'takeBusinessCore');
  const hasSrc = extractFunctionSource(html, 'hasBusinessCore');
  const recSrc = extractFunctionSource(html, 'applyCoreRecordOnto');
  const moveSrc = extractFunctionSource(html, 'recordStockMove');
  const restSrc = extractFunctionSource(html, '_restockFromSale');
  assertTrue(!!restSrc, '_restockFromSale پیدا نشد');
  return new Function('runImpl', runSrc + '\n' + takeSrc + '\n' + hasSrc + '\n' + recSrc + '\n' + moveSrc + '\n' + restSrc + `
    var calls = [];
    var stockMoves = [];
    var ntfCalls = [];
    var parts = [{code:'P1', name:'پره', qty:3}];
    function getSirmanHostSync(){
      return { RunBusiness: function(name, json){
        calls.push(name);
        return runImpl(name, json);
    ...
    return {
      ...
      ntfCalls: ntfCalls,
      parts: parts,
      restock: _restockFromSale
    };
  `)(runImpl);
}

test('R1: موجودی ناموجود روی EXE نباید applyWarehouseDoc را صدا بزند', () => {
  const api = makeExeWarehouseDoc(function(){ return JSON.stringify({ok:false}); }, 'out');
  assertEqual(api.calls.indexOf('inventory.applyWarehouseDoc') < 0, true, 'applyWarehouseDoc نباید صدا شود');
  assertEqual(api.calls.indexOf('inventory.stock') >= 0, true, 'باید پیش‌پرواز inventory.stock باشد');
  assertEqual(api.docs.length, 0, 'سند نباید ثبت شود');
  assertEqual(api.stockMoves.length, 0, 'حرکت نباید ثبت شود');
  assertEqual(api.parts[0].qty, 10, 'qty نباید جهش کند');
});

test('R1: موجودی معتبر باید applyWarehouseDoc موجود را صدا بزند و حرکت NaN نسازد', () => {
  const api = makeExeWarehouseDoc(function(name){
    if (name === 'inventory.stock') {
      return JSON.stringify({ok:true, result:{qty:10, reserved:0, available:10, min:0, reorder:0, price:0}});
    }
    if (name === 'inventory.applyWarehouseDoc') {
      return JSON.stringify({ok:true, result:{ok:true, doc:{id:'WH-OUT-0001', type:'out'}, mutated:{P1:{code:'P1', qty:8, byWh:{'WH-A':8}}}, persistKeys:[]}});
    }
    return JSON.stringify({ok:false});
  }, 'out');
  assertEqual(api.calls.indexOf('inventory.applyWarehouseDoc') >= 0, true, 'applyWarehouseDoc باید صدا شود');
  assertEqual(api.docs.length, 1, 'سند موفق باید ثبت شود');
  api.stockMoves.forEach(function(m){
    assertEqual(typeof m.qty === 'number' && isFinite(m.qty), true, 'qty حرکت نباید NaN باشد');
  });
});

test('R1: HTML-only حواله ورود/خروج موفق و adjust ناموجود باید رفتار قبلی را نگه دارد', () => {
  const saveSrc = extractFunctionSource(html, 'saveWarehouseDoc');
  const hostIdx = saveSrc.indexOf('if(hostOn)');
  const htmlOnly = saveSrc.slice(saveSrc.indexOf('for(var i=0;i<validItems.length;i++)'));
  assertTrue(htmlOnly.indexOf("invReserveOnItem") >= 0, 'رزرو HTML-only باید بماند');
  assertTrue(htmlOnly.indexOf("applyStockByWarehouse") >= 0, 'حرکت HTML-only باید بماند');
  assertTrue(htmlOnly.indexOf('stockDataAvailable') >= 0, 'adjust HTML-only ناموجود باید قطع شود');
  const htmlIn = new Function(
    extractFunctionSource(html, 'stockDataAvailable') + '\n' +
    (extractFunctionSource(html, '_sumByWh') || '') + '\n' +
    extractFunctionSource(html, 'invStockSnapshot') + '\n' +
    extractFunctionSource(html, 'invFindStockItem') + '\n' +
    extractFunctionSource(html, 'invReserveOnItem') + '\n' +
    extractFunctionSource(html, 'applyStockByWarehouse') + `
      function hasBusinessCore(){ return false; }
      function getDefaultWhId(){ return 'WH-A'; }
      function getWhName(id){ return id; }
      function fdt(){ return '1405/05/30'; }
      function _whTag(){ return 'parts'; }
      function svParts(){}
      function svStockMoves(){}
      var stockMoves = [];
      var parts = [{code:'P1', name:'قطعه', qty:10, reserved:0, byWh:{'WH-A':10}, reservedByWh:{}}];
      var inventory = {};
      var defectiveStock = [];
      var out = applyStockByWarehouse('out', 'P1', 'قطعه', 2, 'WH-A', 'warehouse', 'WH-OUT');
      var inn = applyStockByWarehouse('in', 'P1', 'قطعه', 1, 'WH-A', 'warehouse', 'WH-IN');
      return {outOk:out.ok, inOk:inn.ok, qty:parts[0].qty, moves:stockMoves.length};
    `
  )();
  assertEqual(htmlIn.outOk, true, 'خروج HTML-only باید موفق بماند');
  assertEqual(htmlIn.inOk, true, 'ورود HTML-only باید موفق بماند');
  assertEqual(htmlIn.qty, 9, 'qty باید ۱۰-۲+۱=۹ شود');
  void hostIdx;
});

test('R2: addStock موفق باید دقیقاً یک حرکت معتبر بسازد و شکست صفر حرکت بسازد', () => {
  const ok = makeExeRestock(function(name){
    if (name === 'inventory.addStock') {
      return JSON.stringify({ok:true, result:{ok:true, item:{code:'P1', qty:7}}});
    }
    return JSON.stringify({ok:false});
  });
  ok.restock({id:'S-1', items:[{partCode:'P1', qty:4, partName:'پره'}]});
  assertEqual(ok.calls.filter(function(n){ return n === 'inventory.addStock'; }).length, 1, 'باید یک addStock باشد');
  assertEqual(ok.parts[0].qty, 7, 'qty باید از هسته ۷ شود');
  assertEqual(ok.stockMoves.length, 1, 'باید دقیقاً یک حرکت ثبت شود');
  assertEqual(ok.stockMoves[0].qty === 4 || ok.stockMoves[0].qty === -4 || Math.abs(ok.stockMoves[0].qty) === 4, true, 'qty حرکت باید ۴ معتبر باشد');
  assertEqual(typeof ok.stockMoves[0].qty === 'number' && isFinite(ok.stockMoves[0].qty), true, 'حرکت نباید NaN باشد');
  const fail = makeExeRestock(function(){ return JSON.stringify({ok:false}); });
  fail.restock({id:'S-2', items:[{partCode:'P1', qty:4, partName:'پره'}]});
  assertEqual(fail.calls.indexOf('inventory.addStock') >= 0, true, 'شکست باید پس از تلاش addStock باشد');
  assertEqual(fail.parts[0].qty, 3, 'qty نباید عوض شود');
  assertEqual(fail.stockMoves.length, 0, 'شکست addStock نباید حرکت بسازد');
  const htmlRestock = extractFunctionSource(html, '_restockFromSale');
  const htmlOnly = new Function(htmlRestock + `
    function hasBusinessCore(){ return false; }
    var parts = [{code:'P1', name:'پره', qty:3}];
    _restockFromSale({items:[{partCode:'P1', qty:4, partName:'پره'}], status:'final'});
    return parts[0].qty;
  `)();
  assertEqual(htmlOnly, 7, 'HTML-only باید ۴ را به ۳ برگرداند → ۷');
});

test('R3: جهش موفق با stock نامعتبر نباید عدد تفسیر شود و نباید جهش دوم بسازد', () => {
  const runSrc = extractFunctionSource(html, 'runBusinessCore');
  const takeSrc = extractFunctionSource(html, 'takeBusinessCore');
  const hasSrc = extractFunctionSource(html, 'hasBusinessCore');
  const availSrc = extractFunctionSource(html, 'stockDataAvailable');
  ...
  const got = new Function(runSrc + '\n' + takeSrc + '\n' + hasSrc + '\n' + availSrc + '\n' + snapSrc + '\n' + recSrc + '\n' + itemSrc + '\n' + resSrc + '\n' + relSrc + `
    ...
    function getSirmanHostSync(){
      return { RunBusiness: function(name, json){
        ...
        if (name === 'inventory.reserve') {
          mutateCount++;
          return JSON.stringify({ok:true, result:{ok:true, item:{qty:10, reserved:4, reservedByWh:{'WH-A':4}}, stock:{ok:false, reason:'INVENTORY_UNAVAILABLE'}}});
        }
        return JSON.stringify({ok:false});
      }};
    }
    var item = {qty:10, reserved:0, reservedByWh:{}, byWh:{'WH-A':10}};
    var r = invReserveOnItem(item, 4, 'WH-A');
    var numeric = (function(){
      if (!r.ok) return {kind:'fail'};
      if (typeof stockDataAvailable==='function' && !stockDataAvailable(r.stock)) return {kind:'mutated-no-stock', qty:null, available:r.stock && r.stock.available};
      return {kind:'ok', qty:r.stock.available};
    })();
    return {r:r, numeric:numeric, reserved:item.reserved, mutateCount:mutateCount, calls:calls};
  `)();
  assertEqual(got.r.ok, true, 'جهش موفق نباید ok:false دروغین شود');
  assertEqual(got.reserved, 4, 'reserved هسته باید نوشته شده باشد');
  assertEqual(got.mutateCount, 1, 'نباید جهش دوم reserve رخ دهد');
  assertEqual(got.numeric.kind, 'mutated-no-stock', 'caller نباید available را عدد بگیرد');
  assertEqual(got.numeric.qty, null, 'qty عددی نباید از stock نامعتبر خوانده شود');
  assertEqual(got.r.stock && got.r.stock.reason, 'INVENTORY_UNAVAILABLE', 'stock برگشتی باید قرارداد B17 باشد');
  const saveSrc = extractFunctionSource(html, 'saveWarehouseDoc');
  const resCall = saveSrc.indexOf('invReserveOnItem(found.item');
  const after = saveSrc.slice(resCall, resCall + 800);
  assertTrue(after.indexOf('r.stock.available') < 0 && after.indexOf('r.stock.qty') < 0, 'حواله رزرو نباید r.stock را عدد تفسیر کند');
  assertTrue(after.indexOf('it.qty') >= 0, 'حرکت رزرو باید از مقدار سند باشد');
  const failR = new Function(runSrc + '\n' + takeSrc + '\n' + hasSrc + '\n' + availSrc + '\n' + snapSrc + '\n' + recSrc + '\n' + itemSrc + '\n' + resSrc + `
    function getSirmanHostSync(){ return { RunBusiness: function(){ return JSON.stringify({ok:false}); } }; }
    var item = {qty:10, reserved:0, reservedByWh:{}, byWh:{'WH-A':10}};
    return invReserveOnItem(item, 4, 'WH-A');
  `)();
  assertEqual(failR.ok, false, 'شکست جهش باید ok:false باشد');
});
