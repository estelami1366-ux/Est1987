...
3. **لیست مجاز فعلی Host Object:** `Ping`, `CloseApp`, `Notify`, `GetNotifyPort`, `GetBackupDir`, `SaveAppPref`, `LoadAppPref`, `WriteBackupText`, `FinalizeBackup`, `TestRestoreBackup`, `ConsumeBackupSnapshot`, `GetWarrantyBrowseCatalog`, `GetWarrantyBrowseCss`, `ApplyUiSkin`, `GetPrinters`, `PrintHtml`, `PrintDocument`, `GetPrintJob`, `GetMachineInfo`, `GetNetworkInfo`, `SetNetworkConfig`, `WriteWorkspaceFile`, `ReadWorkspaceFile`, `Login`, `Logout`, `BindSession`, `CheckPermission`, `HashPassword`, `VerifyPassword`, `ValidateEntity`, `GetSecurityStatus`, `SaveSecret`, `LoadSecret`, `RunBusiness`, `RunPrintHardwareDiagnostic`, `NewDiagnosticCorrelationId`, `GetRecentDiagnostics`, `GetDiagnosticIncident`, `ExportDiagnosticReport`. هر متد جدید باید به همین شیء اضافه شود، نه با مسیر موازی. `RunBusiness` پل محاسبات/قوانین فاز ۲ است (نه REST). مسیر چاپ تولیدی فقط `GetPrinters` / `PrintHtml` / `PrintDocument` / `GetPrintJob` است و از `IPrintService` می‌گذرد. `RunPrintHardwareDiagnostic` فقط هارنس تشخیص چاپگر است و داده کسب‌وکار را تغییر نمی‌دهد. `NewDiagnosticCorrelationId` / `GetRecentDiagnostics` / `GetDiagnosticIncident` / `ExportDiagnosticReport` مرکز عیب‌یابی P0 هستند (JSONL جدا از چاپ؛ داده کسب‌وکار نمی‌نویسند). عملیات حساس (`SetNetworkConfig`, `WriteWorkspaceFile`, `ReadWorkspaceFile`, `PrintHtml`) از Host Security Gate با نشست و مجوز صفحهٔ موجود عبور می‌کنند. `GetRecentDiagnostics` و `GetDiagnosticIncident` و `ExportDiagnosticReport` به مجوز `Audit.View` نیاز دارند. `WriteBackupText` و `FinalizeBackup` و `TestRestoreBackup` و `ConsumeBackupSnapshot` و `SaveAppPref` برای حفظ داده/ظاهر/پیش‌نمایش فقط‌خواندنی بدون ورود هم مجاز می‌مانند.
