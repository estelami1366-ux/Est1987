...
گردش‌کار تحویل روزانه (نسخه، تست، فایل تک‌تکه، لانچر) در `.agents/skills/laegh-software-workflow/SKILL.md` است. حاکمیت توسعه و حفاظت کد پایدار در `docs/DEVELOPMENT_GOVERNANCE.md` است. دروازهٔ تغییر فاز ۳ در `docs/PHASE_3_CHANGE_GATE.md` است (بدون `PASS` پیاده‌سازی ممنوع). آن قوانین باطل نشده‌اند. این سند جهت معماری را مشخص می‌کند.
...
3. **لیست مجاز فعلی Host Object:** `Ping`, `CloseApp`, `Notify`, `GetNotifyPort`, `GetBackupDir`, `SaveAppPref`, `LoadAppPref`, `WriteBackupText`, `FinalizeBackup`, `TestRestoreBackup`, `ConsumeBackupSnapshot`, `GetWarrantyBrowseCatalog`, `GetWarrantyBrowseCss`, `ApplyUiSkin`, `GetPrinters`, `PrintHtml`, `PrintDocument`, `GetPrintJob`, `GetMachineInfo`, `GetNetworkInfo`, `SetNetworkConfig`, `WriteWorkspaceFile`, `ReadWorkspaceFile`, `Login`, `Logout`, `BindSession`, `CheckPermission`, `HashPassword`, `VerifyPassword`, `ValidateEntity`, `GetSecurityStatus`, `SaveSecret`, `LoadSecret`, `RunBusiness`, `RunPrintHardwareDiagnostic`, `NewDiagnosticCorrelationId`, `GetRecentDiagnostics`, `GetDiagnosticIncident`, `ExportDiagnosticReport`, `OpenDiagnosticCenter`, `ReportUiFault`. هر متد جدید باید به همین شیء اضافه شود، نه با مسیر موازی. `RunBusiness` پل محاسبات/قوانین فاز ۲ است (نه REST). مسیر چاپ تولیدی فقط `GetPrinters` / `PrintHtml` / `PrintDocument` / `GetPrintJob` است و از `IPrintService` می‌گذرد. `RunPrintHardwareDiagnostic` فقط هارنس تشخیص چاپگر است و داده کسب‌وکار را تغییر نمی‌دهد. `NewDiagnosticCorrelationId` / `GetRecentDiagnostics` / `GetDiagnosticIncident` / `ExportDiagnosticReport` مرکز عیب‌یابی P0 هستند (JSONL جدا از چاپ؛ داده کسب‌وکار نمی‌نویسند). `OpenDiagnosticCenter` پنجرهٔ بومی WinForms مرکز تشخیص P2 را باز می‌کند (منطق guidance در Core است). `ReportUiFault` خطای خام UI را می‌پذیرد (P3؛ AlwaysAllowed؛ کد کلاینت نادیده گرفته می‌شود). عملیات حساس (`SetNetworkConfig`, `WriteWorkspaceFile`, `ReadWorkspaceFile`, `PrintHtml`) از Host Security Gate با نشست و مجوز صفحهٔ موجود عبور می‌کنند. `GetRecentDiagnostics` و `GetDiagnosticIncident` و `ExportDiagnosticReport` به مجوز `Audit.View` نیاز دارند. `WriteBackupText` و `FinalizeBackup` و `TestRestoreBackup` و `ConsumeBackupSnapshot` و `SaveAppPref` برای حفظ داده/ظاهر/پیش‌نمایش فقط‌خواندنی بدون ورود هم مجاز می‌مانند.
