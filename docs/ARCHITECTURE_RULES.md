...
3. **لیست مجاز فعلی Host Object:** `Ping`, `CloseApp`, `Notify`, `GetNotifyPort`, `GetBackupDir`, `SaveAppPref`, `LoadAppPref`, `WriteBackupText`, `GetWarrantyBrowseCatalog`, `GetWarrantyBrowseCss`, `ApplyUiSkin`, `GetPrinters`, `PrintHtml`, `GetMachineInfo`, `GetNetworkInfo`, `SetNetworkConfig`, `WriteWorkspaceFile`, `ReadWorkspaceFile`, `Login`, `Logout`, `BindSession`, `CheckPermission`, `HashPassword`, `VerifyPassword`, `ValidateEntity`, `GetSecurityStatus`, `SaveSecret`, `LoadSecret`, `RunBusiness`. هر متد جدید باید به همین شیء اضافه شود، نه با مسیر موازی. `RunBusiness` پل محاسبات/قوانین فاز ۲ است (نه REST). عملیات حساس (`SetNetworkConfig`, `WriteWorkspaceFile`, `ReadWorkspaceFile`, `PrintHtml`) از Host Security Gate با نشست و مجوز صفحهٔ موجود عبور می‌کنند. `WriteBackupText` و `SaveAppPref` برای حفظ داده/ظاهر بدون ورود هم مجاز می‌مانند.
...
8. **شماره نسخه واحد:** فایل `SIRMAN_VERSION.json` در ریشه منبع شماره نسخه است. HTML (`APP_VERSION`) و پوسته ویندوز (`desktop/Directory.Build.props` → `Version`/`InformationalVersion`) باید با همان فایل یکی باشند. حرف یونانی همان روز به رقم چهارم اسمبلی نگاشت می‌شود (α=1 … η=8، θ=9، ι=10، κ=11).
...
10. **Business Core (فاز ۲ / ۲B):** محاسبات و قوانین موجود در `desktop/Sirman.Core/Business` از Host با `RunBusiness` صدا می‌شوند. در exe برای عملیات Migrated نتیجه C# منبع حقیقت است (نه dualPrefer با برد JS). مسیر HTML-only همان توابع JS را به‌عنوان fallback نگه می‌دارد چون Host نیست. ذخیره هنوز localStorage است؛ Core persist نمی‌کند. `saveWar` / `closeWar` / اسناد انبار تا مهاجرت امن DEFERRED می‌مانند.
